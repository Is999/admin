...
var codeHTTPStatusMap = map[int]int{
	CheckMFABind:                 OK,           // 需要先绑定并启用 MFA返回 HTTP 请求成功。
	CheckMFACode:                 OK,           // 需要校验 MFA 设备验证码返回 HTTP 请求成功。
	CheckPasswordReset:           OK,           // 需要先修改登录密码返回 HTTP 请求成功。
	CheckMFAAgain:                BadRequest,   // MFA 校验已过期，需要重新验证返回 HTTP 错误请求。
	...
	RateLimit:                    429,          // 请求过多，限流返回 HTTP 429。
	InternalError:                ServerError,  // 内部错误返回 HTTP 服务器错误。
	DBError:                      ServerError,  // 数据库错误返回 HTTP 服务器错误。
	UserAlreadyExists:            BadRequest,   // 用户已存在返回 HTTP 错误请求。
	AdminRoleAlreadyExists:       BadRequest,   // 后台角色名称已存在返回 HTTP 错误请求。
	AdminPermissionAlreadyExists: BadRequest,   // 后台权限标识已存在返回 HTTP 错误请求。
	InvalidPassword:              BadRequest,   // 密码错误返回 HTTP 错误请求。
	InvalidMFACode:               BadRequest,   // MFA动态验证码错误返回 HTTP 错误请求。
	DependencyUnavailable:        ServiceBusy,  // ready 检查或核心流程发现外部依赖不可用返回 HTTP 服务繁忙。
	MySQLUnavailable:             ServiceBusy,  // MySQL 连接不可用返回 HTTP 服务繁忙。
	RedisUnavailable:             ServiceBusy,  // Redis 连接不可用返回 HTTP 服务繁忙。
	ClickHouseUnavailable:        ServiceBusy,  // ClickHouse 连接不可用返回 HTTP 服务繁忙。
	KafkaUnavailable:             ServiceBusy,  // Kafka 生产链路不可用返回 HTTP 服务繁忙。
	TaskQueueUnavailable:         ServiceBusy,  // 任务队列组件未就绪或不可用返回 HTTP 服务繁忙。
	CollectorUnavailable:         ServiceBusy,  // Collector 组件未就绪或不可用返回 HTTP 服务繁忙。

	UserTagWorkflowLeaseNotFound:      NotFound,    // 用户标签工作流互斥租约不存在，可能已经过期或已释放返回 HTTP 未找到。
	UserTagWorkflowLeaseOwnerMismatch: BadRequest,  // 用户标签工作流互斥租约 owner 与请求 workflowID/mode 不一致返回 HTTP 错误请求。
	UserTagWorkflowLeaseReleaseFailed: ServerError, // 用户标签工作流互斥租约释放过程发生未知失败返回 HTTP 服务器错误。
}
