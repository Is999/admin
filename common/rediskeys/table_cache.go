...
const (
	// TableCacheDataPrefix 表示 table-cache 组件托管业务缓存的项目级 Redis Key 前缀。
	// 该前缀用于隔离 admin-cron 与同 Redis 中其它系统的表缓存数据；最终 key 形如 `admin_cron:table_cache:{appID}:role_tree`。
	TableCacheDataPrefix = "admin_cron:table_cache:"
	// TableCacheDefaultAppID 表示配置缺失 app_id 时的兜底命名空间，避免生成空 appID 前缀。
	TableCacheDefaultAppID = "default"
)
...
func TableCachePrefix(appID string) string {
	...
	if appID == "" {
		appID = TableCacheDefaultAppID
	}
	return TableCacheDataPrefix + appID + ":"
}
