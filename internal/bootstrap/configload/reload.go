...
func hotReloadRestartSpecs() []hotReloadRestartSpec {
	return []hotReloadRestartSpec{
		...
		valueRestartSpec(restartReasonHTTPServer, func(cfg config.Config) any {
			return struct {
				Public   rest.RestConf               // Public 表示公网 HTTP 监听配置。
				Internal config.InternalServerConfig // Internal 表示内网 HTTP 监听配置。
			}{
				Public:   cfg.RestConf,
				Internal: cfg.InternalServer,
			}
		}, preserveBefore(func(effective *config.Config, before config.Config) {
			effective.RestConf = before.RestConf
			effective.InternalServer = before.InternalServer
		})),
		newHotReloadRestartSpec("mode", func(before, after config.Config) bool {
			return reflect.DeepEqual(before.RestConf, after.RestConf) &&
				reflect.DeepEqual(before.InternalServer, after.InternalServer) &&
				strings.TrimSpace(before.Mode) != strings.TrimSpace(after.Mode)
		}, preserveBefore(func(effective *config.Config, before config.Config) {
