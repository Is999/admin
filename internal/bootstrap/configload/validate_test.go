...
// TestValidateBootstrapConfigAcceptsUserRouteShardCount 验证运行时允许已支持的物理拆分档位。
func TestValidateBootstrapConfigAcceptsUserRouteShardCount(t *testing.T) {
	cfg := validAdminBootstrapConfig()
	cfg.User.RouteShardCount = 2
	if err := Validate(cfg); err != nil {
		t.Fatalf("合法 user.route_shard_count 不应失败: %v", err)
	}
}
...
// TestNormalizeBootstrapConfigPreservesInvalidUserValues 确保非法负数不会被默认值掩盖。
func TestNormalizeBootstrapConfigPreservesInvalidUserValues(t *testing.T) {
	cfg := config.Config{}
	cfg.User.RouteShardCount = -1
	cfg.User.ExportSplitRows = -1
	Normalize(&cfg)
	if cfg.User.RouteShardCount != -1 || cfg.User.ExportSplitRows != -1 {
		t.Fatalf("非法用户配置被改写: %+v", cfg.User)
	}
}
...
// TestValidateBootstrapConfigRejectsProductionWildcardInternalHost 确保生产内网监听器不能监听全部网卡。
func TestValidateBootstrapConfigRejectsProductionWildcardInternalHost(t *testing.T) {
	cfg := validAdminBootstrapConfig()
	cfg.Mode = "pro"
	cfg.InternalServer.Host = "0.0.0.0"
	if err := validateInternalServer(cfg.RestConf, cfg.InternalServer, cfg.Mode); err == nil {
		t.Fatal("期望生产 internal_server 通配监听被拒绝，实际为 nil")
	}
}

// TestValidateBootstrapConfigRequiresMTLSForPrivateInternalHost 确保生产跨主机内网监听必须使用 mTLS。
func TestValidateBootstrapConfigRequiresMTLSForPrivateInternalHost(t *testing.T) {
	cfg := validAdminBootstrapConfig()
	cfg.Mode = "pro"
...
// TestValidateBootstrapConfigRejectsInsecureCrossHostAPIService 确保生产跨主机调用 API 不能使用明文 HTTP。
func TestValidateBootstrapConfigRejectsInsecureCrossHostAPIService(t *testing.T) {
	cfg := config.APIServiceConfig{
		InternalBaseURL: "http://10.0.0.20:8891",
		OpsToken:        "test-api-ops-token",
	}
	if err := validateAPIService(cfg, "pro"); err == nil {
		t.Fatal("期望生产跨主机 HTTP API 地址被拒绝，实际为 nil")
	}
	cfg.InternalBaseURL = "https://10.0.0.20:8891"
	cfg.CAFile = "/etc/tls/api-ca.crt"
	cfg.CertFile = "/etc/tls/admin-client.crt"
	cfg.KeyFile = "/etc/tls/admin-client.key"
	if err := validateAPIService(cfg, "pro"); err != nil {
		t.Fatalf("完整 HTTPS mTLS API 配置应通过校验: %v", err)
	}
}

// TestValidateBootstrapConfigRejectsCIDRThatExtendsIntoPublicSpace 确保来源白名单不能覆盖私网之外的地址。
func TestValidateBootstrapConfigRejectsCIDRThatExtendsIntoPublicSpace(t *testing.T) {
	cfg := validAdminBootstrapConfig()
	cfg.Ops.AllowedIPs = []string{"192.168.0.0/15"}
	if err := Validate(cfg); err == nil {
