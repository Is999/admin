...
// TestValidateBootstrapConfigRejectsProductionPlaceholderOpsToken 确保生产内网接口不能使用模板令牌。
func TestValidateBootstrapConfigRejectsProductionPlaceholderOpsToken(t *testing.T) {
	cfg := validAdminProductionBootstrapConfig()
	cfg.Ops.Token = "replace-with-strong-admin-ops-token"
	if err := ValidateProduction(cfg); err == nil {
		t.Fatal("期望生产 ops.token 占位值返回错误，实际为 nil")
	}
}
