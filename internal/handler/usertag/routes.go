	...
	server.AddRoutes([]rest.Route{
		{
			...
			Handler: authMw.Handle(TriggerUserTagWorkflowHandler(serverCtx), shared.UserTagWorkflowTrigger.Alias),
		},
		{
			Method:  http.MethodPost,
			Path:    "/api/user-tags/recalculations", // 指定标签重算
			Handler: authMw.Handle(RecalculateUserTagHandler(serverCtx), shared.UserTagRecalculate.Alias),
		},
		{
			Method:  http.MethodPost,
			Path:    "/api/user-tags/workflow-lease/release", // 手动释放用户标签工作流互斥锁
			Handler: authMw.Handle(ReleaseUserTagWorkflowLeaseHandler(serverCtx), shared.UserTagWorkflowLeaseRelease.Alias),
		},
	})
}
	...
	server.AddRoutes([]rest.Route{
		{
			...
			Handler: internalMw.Handle(TriggerUserTagWorkflowHandler(serverCtx), shared.UserTagWorkflowTrigger.Alias),
		},
		{
			Method:  http.MethodPost,
			Path:    "/internal/user-tags/recalculations", // 内网指定标签重算
			Handler: internalMw.Handle(RecalculateUserTagHandler(serverCtx), shared.UserTagRecalculate.Alias),
		},
	})
}
