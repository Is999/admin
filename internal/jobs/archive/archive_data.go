...
func (s *Service) ensureHistoryTable(ctx context.Context, db *gorm.DB, job jobConfig, historyTable string) error {
	exists, err := tableExists(ctx, db, historyTable)
	if err != nil {
		return errors.Wrapf(err, "检查归档历史表失败 job=%s table=%s", job.Name, historyTable)
	}
	if exists {
		return nil
	}
...
func (s *Service) archiveUpperBound(ctx context.Context, job jobConfig, controlDB *gorm.DB) (time.Time, bool) {
	upperBound := s.archiveEligibleUntil(job, time.Now())
...
// archiveEligibleUntil 计算指定时刻已满足安全延迟和归档延迟的排他上界。
func (s *Service) archiveEligibleUntil(job jobConfig, now time.Time) time.Time {
