...
func (s *Service) processDeleteSegment(ctx context.Context, job jobConfig, sourceDB *gorm.DB, controlDB *gorm.DB, segment *Segment, workerID string) (result deleteSegmentResult, err error) {
	...
	historyTableExists, tableErr := tableExists(ctx, sourceDB, segment.HistoryTableName)
	if tableErr != nil {
		return result, errors.Wrapf(tableErr, "检查归档历史表失败 job=%s table=%s", job.Name, segment.HistoryTableName)
	}
