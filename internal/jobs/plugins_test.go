...
// TestExcelExportPluginRegistersSysConfigBackupCleanup 验证字典导入前备份清理任务已接入默认任务运行时。
func TestExcelExportPluginRegistersSysConfigBackupCleanup(t *testing.T) {
	runtime, manager, cleanup := newTestRuntime(t, config.Config{})
	defer cleanup()

	if err := runtime.RegisterPlugins(ExcelExportPlugin()); err != nil {
		t.Fatalf("注册 Excel 导出插件失败: %v", err)
	}
	if !taskTypeRegistered(manager, types.SysConfigExcelBackupCleanupTaskType) {
		t.Fatalf("未注册字典导入前备份清理 handler %s，当前清单=%+v",
			types.SysConfigExcelBackupCleanupTaskType, manager.ListRegisteredTaskTypes(context.Background()))
	}
}
