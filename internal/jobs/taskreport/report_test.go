...
// TestBuildRecordsScanProgress 验证日报按页扫描时会持续写入任务统计器，而不是只在汇总完成后记录。
func TestBuildRecordsScanProgress(t *testing.T) {
	req := ReportRequest{
		WindowStart: time.Date(2026, 7, 3, 10, 0, 0, 0, time.UTC),
		WindowEnd:   time.Date(2026, 7, 4, 10, 0, 0, 0, time.UTC),
		GeneratedAt: time.Date(2026, 7, 4, 10, 0, 5, 0, time.UTC),
	...
	manager := &reportFakeManager{
		...
		tasks: map[string][]types.TaskItem{
			asynq.TaskStateCompleted.String(): {
				reportTestItem("task-progress", "demo:periodic", asynq.TaskStateCompleted.String(), req.WindowEnd.Add(-time.Second).Format(time.RFC3339), map[string]string{
					taskwire.HeaderTaskSource:   taskwire.WorkflowSourcePeriodic,
					taskwire.HeaderPeriodicName: "demo-periodic",
				}, 0, ""),
			},
	...
	ctx, tracker := taskstats.WithTracker(context.Background(), TraceNameDailySummary)
	if _, err := NewService(manager).Build(ctx, req); err != nil {
		t.Fatalf("构建日报失败: %v", err)
	}
	snapshot := tracker.Snapshot()
	wantName := taskstats.JoinDetailName(TraceNameDailySummary, "scanned_tasks")
	for _, detail := range snapshot.Details {
		if detail.Name == wantName && detail.Action == taskstats.ActionRead {
			if detail.Count != reportPageSize || detail.Times != 1 {
				t.Fatalf("日报扫描进度异常: %+v", detail)
			}
			return
		}
	}
	t.Fatalf("日报扫描进度未写入任务统计器: %+v", snapshot.Details)
}
