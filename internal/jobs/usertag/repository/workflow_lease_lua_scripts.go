...
//
//go:embed user_tag_workflow_lease_renew.lua
...
//
//go:embed user_tag_workflow_lease_release.lua
var userTagWorkflowLeaseReleaseScriptText string

// userTagWorkflowShardDoneScriptText 保存 sync_kafka 分片完成屏障 Lua 脚本源码。
// 每个 workflow_id 独立使用一个 sync_done 集合，避免不同工作流之间互相影响。
//
//go:embed user_tag_workflow_shard_done.lua
...
var (
	...
	// userTagWorkflowShardDoneScript 记录一个 sync_kafka 分片完成并返回当前已完成分片数。
