...
type WorkflowPayload struct {
	WorkflowID       string   `json:"workflow_id"`                  // 工作流 ID，用于日志链路、运行期表和 outbox 幂等
	...
	SyncSnapshotOnly bool     `json:"sync_snapshot_only,omitempty"` // 是否只刷新同步快照，不推 Kafka 消息
}
...
type RuntimeOptions struct {
	...
	SyncSnapshotOnly     bool    // 是否只刷新同步快照
...
// Kafka outbox 只允许在最终标签态完成后生成该事件，不能在 stage 内提前推送。
