...
func buildAdminInfoCache(admin *model.Admin, token string) *types.AdminInfo {
	return buildAdminProfileCache(admin).ToAdminInfo(token)
}
...
func (l *AdminLogic) Login(req *types.LoginReq) *types.BizResult {
	...
	_ = cacheLogic.RdsDelKeys(cacheLogic.getAdminLogoutTokenKey(admin.ID))

	userInfo, err := (&UserCompatLogic{BaseLogic: l.BaseLogic}).BuildCompatUserInfo(admin, token)
	if err != nil {
		return types.ServerError(i18n.MsgKeyInternalErrorFormat, err,
...
func (l *AdminLogic) Create(req *types.AddAdminReq) *types.BizResult {
	if err := l.requireOperateMFATwoStep(MFAScenarioAddUser, req.TwoStepKey, req.TwoStepValue); err != nil {
	...
	if strings.TrimSpace(req.MfaSecureKey) != "" {
		encryptedMFASecret, err = NewSecurityLogic(l.Context(), l.svc).encryptAdminMFASecret(req.MfaSecureKey)
		if err != nil {
	...
	if len(roleIDs) > 0 {
		if err := (&AdminRoleLogic{BaseLogic: l.BaseLogic}).ensureRolesWithinManageScope(roleIDs); err != nil {
...
func (l *AdminLogic) mfaBizResult(err error) *types.BizResult {
	...
	if errors.Is(err, ErrAdminMFATwoStepExpired) || errors.Is(err, ErrAdminMFARequired) {
		return types.NewBizResult(codes.CheckMFAAgain).
			...
			WithError(wrapLogicError(err, "AdminLogic.mfaBizResult MFA二次校验已失效"))
	}
	if errors.Is(err, types.Nil) {
		return types.NewBizResult(codes.Unauthorized).
			SetI18nMessage(i18n.MsgKeyNeedLogin).
			WithError(wrapLogicError(err, "AdminLogic.mfaBizResult 当前请求未登录"))
	}
	return types.NewBizResult(codes.DBError).
		SetI18nMessage(i18n.MsgKeyDBError).
		WithError(wrapLogicError(err, "AdminLogic.mfaBizResult MFA校验失败"))
}
...
func (l *AdminLogic) GetAdminProfileByID(id int) (*types.AdminProfile, error) {
	...
	result, err := manager.LoadThrough(l.Context(), tableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keys.AdminProfile, id)), profile, nil)
	if err != nil {
		return nil, errors.Tag(err)
	}
...
func (l *AdminLogic) GetLoginAfterInfo(ctxAdmin *helper.CtxAdmin) *types.BizResult {
	info, err := NewCacheLogic(l.Context(), l.svc).GetAdminInfo(ctxAdmin.ID)
	if err != nil {
		...
		// 缓存用户信息
		if err = NewCacheLogic(l.Context(), l.svc).SetAdminInfo(ctxAdmin.ID, info); err != nil {
			return &types.BizResult{
	...
	roles, err := (&AdminRoleRelLogic{BaseLogic: l.BaseLogic}).GetRolesByUserID(int64(ctxAdmin.ID))
	if err != nil {
		return &types.BizResult{
			Code:       codes.DBError,
			MessageKey: i18n.MsgKeyRoleFetchFail,
