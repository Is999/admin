...
func TestRefreshAccessTokenRenewsSessionTTL(t *testing.T) {
	...
	if err := cacheLogic.SetAdminSession(1, &types.AdminSession{ID: 1, UserName: "admin", Token: "old-token"}); err != nil {
	...
	ctx = cachelogic.WithAdminSession(ctx, &types.AdminSession{ID: 1, UserName: "admin", Token: "old-token"})
	...
	info, err := cacheLogic.GetAdminSession(1)
	if err != nil {
	...
	ttl := rds.TTL(context.Background(), keys.AdminSessionRedisKey(1)).Val()
...
func TestRefreshAccessTokenRejectsStaleRequestToken(t *testing.T) {
	...
	svcCtx := svc.NewServiceContext(cfg, svc.Dependencies{Rds: rds})
	cacheLogic := cachelogic.NewCacheLogic(context.Background(), svcCtx)
	if err := cacheLogic.SetAdminSession(1, &types.AdminSession{ID: 1, UserName: "admin", Token: "current-token"}); err != nil {
		t.Fatalf("SetAdminSession() error = %v", err)
	}

	ctx, _ := requestctx.New(context.Background())
	requestctx.SetAccessToken(ctx, "stale-token")
	ctx = cachelogic.WithAdminSession(ctx, &types.AdminSession{ID: 1, UserName: "admin", Token: "current-token"})
	logicObj := &AdminLogic{BaseLogic: corelogic.NewBaseLogicWithContext(ctx, svcCtx)}
	result := logicObj.RefreshAccessToken(&helper.CtxAdmin{ID: 1, Name: "admin"})
	if result == nil || result.Code != 401 {
...
func TestGetLoginAfterInfoRejectsMissingSession(t *testing.T) {
	...
	if server.Exists(keys.AdminSessionRedisKey(1)) {
