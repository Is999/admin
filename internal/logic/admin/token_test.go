...
// TestGetUserPermissionCodesReportsMissingRedis 验证权限码读取不会在 Redis 缺失时回退数据库。
func TestGetUserPermissionCodesReportsMissingRedis(t *testing.T) {
	svcCtx := svc.NewServiceContext(config.Config{}, svc.Dependencies{})
	logicObj := &AdminLogic{BaseLogic: corelogic.NewBaseLogicWithContext(context.Background(), svcCtx)}

	result := logicObj.GetUserPermissionCodes(7)
	if result == nil || result.Code != codes.RedisUnavailable {
		t.Fatalf("GetUserPermissionCodes() result=%+v, want code=%d", result, codes.RedisUnavailable)
	}
}
