...
func (l *AdminManageLogic) List(req *types.AdminListReq) *types.BizResult {
	...
	list, total, err := model.List[model.Admin](dbq, req.Page, req.PageSize, orderBy, normalizedOrderDirection(req.Order))
	if err != nil {
...
func (l *AdminManageLogic) Update(req *types.UpdateAdminReq) *types.BizResult {
	mfaScenario := MFAScenarioEditUser
	if req.Password != nil && strings.TrimSpace(*req.Password) != "" {
		mfaScenario = MFAScenarioChangePassword
	}
...
func (l *AdminManageLogic) Delete(req *types.IDPathReq) *types.BizResult {
	if err := l.requireOperateMFATwoStep(MFAScenarioDeleteUser, req.TwoStepKey, req.TwoStepValue); err != nil {
		return l.mfaBizResult(err)
	}
...
func (l *AdminManageLogic) UpdateStatus(req *types.AdminStatusReq) *types.BizResult {
	if err := l.requireOperateMFATwoStep(MFAScenarioUserStatus, req.TwoStepKey, req.TwoStepValue); err != nil {
		return l.mfaBizResult(err)
	}
...
func (l *AdminManageLogic) ResetPassword(req *types.ResetAdminPasswordReq) *types.BizResult {
	if err := l.requireOperateMFATwoStep(MFAScenarioResetUserPassword, req.TwoStepKey, req.TwoStepValue); err != nil {
		return l.mfaBizResult(err)
	}
	if req.ID == l.GetCtxAdmin().ID {
		return types.NewBizResult(codes.Fail).
...
func (l *AdminManageLogic) ResetInitialState(req *types.ResetAdminInitialStateReq) *types.BizResult {
	if err := l.requireOperateMFATwoStep(MFAScenarioResetUserInitialState, req.TwoStepKey, req.TwoStepValue); err != nil {
		return l.mfaBizResult(err)
	}
	if req.ID == l.GetCtxAdmin().ID {
		return types.NewBizResult(codes.Fail).
	...
	invalidateAdminRelationCache(l.BaseLogic, req.ID)
	_ = NewSecurityLogic(l.Context(), l.svc).ClearLoginMFACompleted(req.ID)
	if err := NewSecurityLogic(l.Context(), l.svc).ClearAdminMFATwoStepTickets(req.ID); err != nil {
		logWrappedError(l.Logger, err, "AdminManageLogic.ResetInitialState 清理管理员ID[%d]MFA二次票据失败", req.ID)
	}
	return types.NewBizResult(codes.UpdateSuccess).
		SetI18nMessage(i18n.MsgKeyUpdateSuccess)
}
...
func (l *AdminManageLogic) ReplaceRoles(req *types.AdminRoleAssignReq) *types.BizResult {
	if err := l.requireOperateMFATwoStep(MFAScenarioEditUser, req.TwoStepKey, req.TwoStepValue); err != nil {
		return l.mfaBizResult(err)
	}
...
func (l *AdminManageLogic) pruneInheritedAssignedRoleIDs(roleIDs []int) ([]int, error) {
	...
	for _, row := range rows {
		...
		for parentRoleID := range selectedSet {
			...
			if containsTreeID(row.Pids, parentRoleID) {
...
// buildAdminUpdates 根据请求和旧模型构建管理员字段更新集合。
...
func (l *AdminManageLogic) replaceAdminRolesTx(tx *gorm.DB, adminID int, roleIDs []int) error {
	...
	if err := tx.Where("user_id = ?", adminID).Delete(&model.AdminRoleRel{}).Error; err != nil {
		return errors.Wrap(err, "清理管理员旧角色关系失败")
	}
	if len(roleIDs) == 0 {
		return nil
	}
