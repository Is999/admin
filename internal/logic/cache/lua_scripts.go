...
// setAdminSessionFieldsScriptText 保存管理员会话字段批量更新 Lua 脚本源码。
// 脚本会先确认缓存 key 和全部目标字段均存在，再原子 HSET，避免补写或部分更新不完整缓存。
//
//go:embed assets/set_admin_session_fields.lua
var setAdminSessionFieldsScriptText string

// setAdminSessionFieldsScript 原子更新管理员会话中已经存在的受控字段。
var setAdminSessionFieldsScript = redis.NewScript(embedasset.StripLeadingLineComments(setAdminSessionFieldsScriptText, "--"))
...
// deleteAdminSessionForLogoutScript 原子校验退出 token 并删除管理员会话。
...
// clearSecurityCacheBarrierScriptText 保存安全缓存阻断键的 CAS 清理 Lua 脚本源码。
// 脚本仅在阻断键仍与读取快照一致时清理，避免删除并发写入的新阻断状态。
//
//go:embed assets/clear_security_cache_barrier.lua
var clearSecurityCacheBarrierScriptText string

// clearSecurityCacheBarrierScript 原子校验并清理安全缓存阻断键。
var clearSecurityCacheBarrierScript = redis.NewScript(embedasset.StripLeadingLineComments(clearSecurityCacheBarrierScriptText, "--"))
