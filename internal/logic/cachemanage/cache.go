...
type cacheSearchStats struct {
	...
	scanRounds     int    // 保留日志字段；当前缓存搜索不再执行 Redis SCAN，固定为 0。
	scanNodeCount  int    // 保留日志字段；当前缓存搜索不再遍历 Redis Cluster 节点，固定为 0。
...
// 2. 模板缓存额外支持按模板固定前缀做“前缀模糊查询”，例如管理员信息模板。
func matchCacheListItem(item types.CacheItem, keyword string) bool {
	...
	return matchTemplatePrefix(item.Key, keyword)
}
...
func (l *SystemCacheLogic) ServerInfo() *types.BizResult {
	...
	info, err := l.Redis().Info(l.Ctx).Result()
	if err != nil {
...
// 内置缓存支持输入逻辑 key，但详情读取必须落到当前 app_id 命名空间下的真实 key。
func (l *SystemCacheLogic) cacheInfoLookupKey(key string) string {
	item := l.matchCacheItem(key)
	if l.cacheItemUsesTableCache(item) {
		return cachelogic.TableCachePhysicalKey(l.BaseLogic, key)
	}
	if l.cacheItemUsesAppScope(item) {
		return l.AppRedisKey(key)
	}
...
func (l *SystemCacheLogic) SearchKey(req *types.CacheKeyReq) *types.BizResult {
	...
	if err := l.ensureCurrentAppCacheKey(req.Key); err != nil {
		return types.Forbidden(i18n.MsgKeyForbidden).ToBizResult().
			WithError(corelogic.WrapLogicError(err, "SystemCacheLogic.SearchKey 禁止跨站点缓存key[%s]", req.Key))
	}
	...
	if normalizeErr != nil {
		return types.NewBizResult(codes.ParamError).
			...
			WithError(corelogic.WrapLogicError(normalizeErr, "SystemCacheLogic.SearchKey 缓存搜索参数校验失败"))
	}
	if validateErr := l.validateSearchPattern(pattern); validateErr != nil {
		return types.NewBizResult(codes.ParamError).
			...
			WithError(corelogic.WrapLogicError(validateErr, "SystemCacheLogic.SearchKey 缓存搜索模式校验失败"))
	}
	start := time.Now()
	keys, searchPath, stats, err := l.searchKeys(pattern, cacheSearchMaxResults)
	if err != nil {
		logCacheInfo(l.Ctx, "缓存搜索 失败",
...
func (l *SystemCacheLogic) searchKeys(pattern string, maxResults int) ([]string, string, cacheSearchStats, error) {
	...
	if err := l.ensureCurrentAppCacheKey(pattern); err != nil {
		return nil, "foreign_app_key", cacheSearchStats{}, errors.Tag(err)
	}
...
// 先保留用户输入的真实 key；若命中 table-cache 托管目标，再补充新命名空间 key，支持逻辑 key 输入。
...
func (l *SystemCacheLogic) Renew(req *types.CacheRenewReq) *types.BizResult {
	...
	if err := l.ensureCurrentAppCacheKey(req.Key); err != nil {
		return types.Forbidden(i18n.MsgKeyForbidden).ToBizResult().
			WithError(corelogic.WrapLogicError(err, "SystemCacheLogic.Renew 禁止跨站点缓存key[%s]", req.Key))
	}
	if !l.isBuiltinRefreshableKey(req.Key) {
		err := errors.Errorf("非内置缓存key禁止刷新")
		return types.Forbidden(i18n.MsgKeyForbidden).ToBizResult().
			WithError(corelogic.WrapLogicError(err, "SystemCacheLogic.Renew 禁止刷新非内置缓存key[%s]", req.Key))
	}
...
func (l *SystemCacheLogic) RenewAll() *types.BizResult {
	...
	if err := manager.RefreshAll(l.Ctx); err != nil {
		return types.ServerError(i18n.MsgKeyCacheInfoFail, err,
...
func (l *SystemCacheLogic) RefreshCacheByKey(key string) error {
	...
	key = strings.TrimSpace(key)
	if err := l.ensureCurrentAppCacheKey(key); err != nil {
	...
	if l.cacheItemUsesTableCache(item) {
		...
		physicalKey := cachelogic.TableCachePhysicalKey(l.BaseLogic, key)
		if err := manager.RefreshByKey(l.Ctx, physicalKey); err == nil {
...
func (l *SystemCacheLogic) cacheItems() []types.CacheItem {
	tableItems := cachelogic.TableCacheItems(l.BaseLogic)
...
func (l *SystemCacheLogic) cacheItemUsesTableCache(item *types.CacheItem) bool {
	...
	for _, target := range cachelogic.TableCacheItems(l.BaseLogic) {
...
func (l *SystemCacheLogic) previewHashValue(ctx context.Context, key string, maxItems int) (map[string]string, int64, error) {
	...
	if err != nil {
		// 抽样预览属于辅助信息；命令不支持或抽样失败时只返回总数，禁止回退 HSCAN/HGETALL 放大 Redis 压力。
		return map[string]string{}, total, nil
	}
...
func (l *SystemCacheLogic) matchCacheItem(key string) *types.CacheItem {
	...
	physicalKey := cachelogic.TableCachePhysicalKey(l.BaseLogic, key)
	logicalKey := cachelogic.TableCacheLogicalKey(l.BaseLogic, key)
	appLogicalKey := l.currentAppLogicalKey(key)
	items := l.cacheItems()
	for i := range items {
		if items[i].Key == key ||
...
// table-cache 托管模板允许用户输入逻辑模板，同时按新物理模板匹配枚举出的真实 Redis key。
func (l *SystemCacheLogic) cacheSearchCandidateMatches(pattern string, candidate string, target *templateSearchTarget) bool {
	...
	if target != nil && target.tableCache {
		physicalPattern := cachelogic.TableCachePhysicalKey(l.BaseLogic, pattern)
		if physicalPattern != "" {
...
func (l *SystemCacheLogic) filterExistingKeys(candidates []string, maxResults int) ([]string, cacheSearchStats, error) {
	...
	ctx, cancel := context.WithTimeout(l.Ctx, cacheSearchExistsTimeout)
	defer cancel()
...
func (l *SystemCacheLogic) keyExists(key string) (bool, error) {
	count, err := l.Redis().Exists(l.Ctx, strings.TrimSpace(key)).Result()
	if err != nil {
...
// cacheSearchPatternMatch 使用 Redis glob 规则匹配候选 key。
