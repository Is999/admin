...
func TestMaskCacheValueForSecretKeyHashes(t *testing.T) {
	value := map[string]string{
		"aes_iv_ref":  "/etc/admin/keys/demo/aes_iv",
		"aes_key_ref": "/etc/admin/keys/demo/aes_key",
		"status":      "1",
		"version":     "v1",
	}
...
func TestFilterExistingKeysCapsResultAllocationAndDeduplicates(t *testing.T) {
	server := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: server.Addr()})
	for _, key := range []string{"cache:item:1", "cache:item:2", "cache:item:3"} {
...
func TestMatchSearchTemplateTarget(t *testing.T) {
	...
	wantConfigTemplate := cachelogic.TableCachePhysicalKey(logicObj.BaseLogic, keys.SysConfigUUIDPattern)
...
// TestCacheSearchCandidateMatchesLogicalTableCachePattern 验证逻辑模板能匹配新版 table-cache 物理 key。
func TestCacheSearchCandidateMatchesLogicalTableCachePattern(t *testing.T) {
	...
	candidate := cachelogic.TableCachePhysicalKey(logicObj.BaseLogic, "config_uuid:site.name")
...
func TestCacheTemplateDefinitionsUsePhysicalKeys(t *testing.T) {
	...
	for _, item := range logicObj.cacheItems() {
		...
		if item.IsTemplate {
			...
			logicalExampleKey := cachelogic.TableCacheLogicalKey(logicObj.BaseLogic, item.ExampleKey)
...
// TestForeignAppCacheKeyRejected 验证缓存管理接口拒绝其它 app_id 的完整 Redis key。
func TestForeignAppCacheKeyRejected(t *testing.T) {
	server := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: server.Addr()})
	logicObj := &SystemCacheLogic{
		BaseLogic: corelogic.NewBaseLogicWithContext(context.Background(), svc.NewServiceContext(config.Config{AppID: "site-a"}, svc.Dependencies{Rds: client})),
	...
	foreignTableKey := "app:site-b:table:role_tree"
	if logicObj.matchCacheItem(foreignTableKey) != nil {
		t.Fatal("其它 app_id 的 table-cache key 不应匹配本站点内置缓存项")
	}
	if err := logicObj.RefreshCacheByKey(foreignTableKey); err == nil || !strings.Contains(err.Error(), "其它app_id") {
	...
	if got := logicObj.KeyInfo(&types.CacheKeyReq{Key: foreignSessionKey}); got == nil || got.Code != codes.Forbidden {
		t.Fatalf("KeyInfo(%s) code = %v, want forbidden", foreignSessionKey, got)
	}
	if got := logicObj.SearchKey(&types.CacheKeyReq{Key: foreignSessionKey}); got == nil || got.Code != codes.Forbidden {
		t.Fatalf("SearchKey(%s) code = %v, want forbidden", foreignSessionKey, got)
	}
	if got := logicObj.Renew(&types.CacheRenewReq{Key: foreignSessionKey}); got == nil || got.Code != codes.Forbidden {
		t.Fatalf("Renew(%s) code = %v, want forbidden", foreignSessionKey, got)
	}
}

// TestSearchKeysExactLogicalTableCacheKeyFindsPhysicalKey 验证精确搜索逻辑 key 时会读取新版物理 key。
func TestSearchKeysExactLogicalTableCacheKeyFindsPhysicalKey(t *testing.T) {
	...
	physicalKey := cachelogic.TableCachePhysicalKey(logicObj.BaseLogic, keys.RoleStatus)
...
func TestKeyInfoLogicalAdminInfoKeyReadsAppPrefix(t *testing.T) {
	...
	data, ok := result.Data.(*types.CacheKeyInfoResp)
	if !ok {
		t.Fatalf("KeyInfo() data type = %T, want *types.CacheKeyInfoResp", result.Data)
	}
	if data.Key != physicalKey {
		t.Fatalf("KeyInfo() key = %s, want %s", data.Key, physicalKey)
	}
}
