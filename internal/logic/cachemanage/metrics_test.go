...
func TestBuildCacheMetricsResp(t *testing.T) {
	...
	for index := 0; index < 8; index++ {
		metrics.RecordCacheHit(ctx, "admin_role_ids")
	}
	for index := 0; index < 2; index++ {
		metrics.RecordCacheMiss(ctx, "admin_role_ids")
	}
	// 细分读取状态不参与基础命中率，避免与 cache_hit/cache_miss 重复计数。
	metrics.RecordLookupState(ctx, "admin_role_ids", tablecache.LookupStateEmpty)
	for index := 0; index < 3; index++ {
		metrics.RecordRefresh(ctx, "admin_role_ids", "success", time.Millisecond)
	}
	metrics.RecordRefresh(ctx, "admin_role_ids", "error", time.Millisecond)
	metrics.RecordLoaderError(ctx, "admin_role_ids", context.Canceled)
	metrics.RecordLockFailed(ctx, "admin_role_ids")
	metrics.RecordWaitTimeout(ctx, "admin_role_ids")
	metrics.RecordScanFallback(ctx, "admin_role_ids", "admin_role_ids:")
	...
	result := buildCacheMetricsResp(families, []types.CacheItem{
		{Index: "admin_role_ids", KeyTitle: "admin_role_ids:%d", Remark: "管理员角色 ID", Category: "auth"},
	...
	if result.Targets[0].Index != "admin_role_ids" || result.Targets[0].HitTotal != 8 {
