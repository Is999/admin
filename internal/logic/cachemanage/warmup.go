...
func (l *SystemCacheLogic) warmupTemplateTargets() []warmupTemplateTarget {
	return []warmupTemplateTarget{
		...
		},
		{
			...
			buildKeys: func(l *SystemCacheLogic) ([]string, error) {
				return l.buildTableRoleKeys(keys.RolePermission)
			},
		},
		{
			templateKey: cachelogic.TableCachePhysicalKey(l.BaseLogic, keys.RoleDocPermissionPattern),
			buildKeys: func(l *SystemCacheLogic) ([]string, error) {
				return l.buildTableRoleKeys(keys.RoleDocPermission)
			},
		},
	}
}

// buildTableRoleKeys 基于启用角色 ID 枚举指定 table-cache 模板的实例 Key。
func (l *SystemCacheLogic) buildTableRoleKeys(keyFormat string) ([]string, error) {
	...
	for _, roleID := range roleIDs {
		if roleID > 0 {
			keysList = append(keysList, cachelogic.TableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keyFormat, roleID)))
		}
	}
	return keysList, nil
}
