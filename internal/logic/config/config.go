...
// NewSysConfigLogicWithContext 创建复用已有 context 的系统常量配置逻辑对象。
func NewSysConfigLogicWithContext(ctx context.Context, svcCtx *svc.ServiceContext) *SysConfigLogic {
	return &SysConfigLogic{
...
func (l *SysConfigLogic) List(req *types.SysConfigListReq) *types.BizResult {
	...
	list, total, err := model.List[model.SysConfig](dbq, req.GetPageReq.Page, req.PageSize, orderBy, corelogic.NormalizedOrderDirection(req.Order))
	if err != nil {
...
func (l *SysConfigLogic) Create(req *types.SaveSysConfigReq) *types.BizResult {
	...
	if err != nil {
		return types.ParamErrorResult(err).
			WithError(corelogic.WrapLogicError(err, "SysConfigLogic.Create 参数校验失败"))
	}
...
func (l *SysConfigLogic) Update(req *types.SaveSysConfigReq) *types.BizResult {
	...
	if old.Type != req.Type {
		typeErr := errors.Errorf("配置类型不允许修改")
		return types.ParamErrorResult(typeErr).
			WithError(corelogic.WrapLogicError(typeErr, "SysConfigLogic.Update 参数校验失败"))
	}

	valueRaw, err := req.ValueRawMessage()
	if err != nil {
		return types.ParamErrorResult(err).
			WithError(corelogic.WrapLogicError(err, "SysConfigLogic.Update 参数校验失败"))
	}
	exampleRaw, err := req.ExampleRawMessage()
	if err != nil {
		return types.ParamErrorResult(err).
...
func (l *SysConfigLogic) GetCachedValue(uuid string) (any, error) {
	...
	result, err := manager.LoadThrough(l.Ctx, cachelogic.TableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keys.SysConfigUUID, uuid)), &cache, nil)
	if err != nil {
		return nil, errors.Tag(err)
	}
	...
	if corelogic.CacheIsEmptyMarker(cache["value"]) {
		return nil, redis.Nil
	}
...
func (l *SysConfigLogic) getCacheHash(uuid string) (map[string]string, error) {
	...
	key := cachelogic.TableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keys.SysConfigUUID, uuid))
	return l.Redis().HGetAll(l.Ctx, key).Result()
}
...
func (l *SysConfigLogic) RenewByUUID(uuid string) error {
	...
	return manager.RefreshByKey(l.Ctx, cachelogic.TableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keys.SysConfigUUID, uuid)))
}
...
func sysConfigModelToItem(cfg model.SysConfig) types.SysConfigItem {
	return types.SysConfigItem{
		...
		Version:   cfg.Version,
		Editable:  1,
		CreatedAt: corelogic.FormatDateTime(cfg.CreatedAt),
