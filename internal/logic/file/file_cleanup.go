...
// ConsumeImportedObject 在配置导入成功后失效上传会话并删除已经消费的对象。
func (l *FileTransferLogic) ConsumeImportedObject(session *transfer.UploadSession) error {
	...
	if err := uploadManager.DeleteSession(l.Ctx, session); err != nil {
		return errors.Wrap(err, "失效已消费上传会话失败")
	}
