...
// TestConsumeImportedObjectInvalidatesSessionBeforeDeletingObject 验证导入消费后不会留下可复用的失效会话。
func TestConsumeImportedObjectInvalidatesSessionBeforeDeletingObject(t *testing.T) {
	redisServer := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: redisServer.Addr()})
	t.Cleanup(func() { _ = client.Close() })
	...
	objectRoot := t.TempDir()
	objectKey := "sys-config-excel-import/202607/26/import.xlsx"
	objectPath := filepath.Join(objectRoot, filepath.FromSlash(objectKey))
	if err := os.MkdirAll(filepath.Dir(objectPath), 0o755); err != nil {
		t.Fatalf("创建测试对象目录失败: %v", err)
	}
	...
	logicObj := NewFileTransferLogicWithContext(context.Background(), svcCtx)
	if err := logicObj.ConsumeImportedObject(session); err != nil {
		t.Fatalf("消费导入对象失败: %v", err)
	}
	if exists, err := uploadManager.HasSession(context.Background(), session.UploadID); err != nil || exists {
		t.Fatalf("已消费上传会话仍然存在 exists=%t err=%v", exists, err)
	}
	if _, err := os.Stat(objectPath); !os.IsNotExist(err) {
		t.Fatalf("已消费上传对象仍然存在 err=%v", err)
	}
}
