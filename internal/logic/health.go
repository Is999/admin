...
type HealthLogic struct {
	*BaseLogic // BaseLogic 提供统一上下文、日志和 ServiceContext 访问能力。
}

// NewHealthLogic 创建健康检查 logic。
func NewHealthLogic(ctx context.Context, svcCtx *svc.ServiceContext) *HealthLogic {
	return &HealthLogic{BaseLogic: NewBaseLogicWithContext(ctx, svcCtx)}
}
...
func (l *HealthLogic) Readiness(ctx context.Context) (*types.HealthStatusResp, error) {
	...
	if l.service() == nil {
		appendStatus(dependencyError("service_context", codes.DependencyUnavailable, "ServiceContext未初始化"))
	} else {
	...
	if firstErr != nil {
		...
		return resp, errors.Tag(firstErr)
	}
	return resp, nil
}
...
func (l *HealthLogic) checkGormDB(ctx context.Context, name string, db *gorm.DB, code int) (types.HealthDependencyStatus, error) {
	if db == nil {
		return dependencyError(name, code, "数据库连接未初始化")
	}
	sqlDB, err := db.DB()
	if err != nil {
		return dependencyError(name, code, fmt.Sprintf("数据库连接池不可用: %v", err))
	}
...
func (l *HealthLogic) checkRedis(ctx context.Context) (types.HealthDependencyStatus, error) {
	if l.service() == nil || l.service().Rds == nil {
		return dependencyError("redis", codes.RedisUnavailable, "Redis客户端未初始化")
	}
	checkCtx, cancel := context.WithTimeout(ctx, healthCheckTimeout)
	defer cancel()
	if err := l.service().Rds.Ping(checkCtx).Err(); err != nil {
		return dependencyError("redis", codes.RedisUnavailable, fmt.Sprintf("Redis PING失败: %v", err))
	}
...
func (l *HealthLogic) checkKafka(enabled bool) (types.HealthDependencyStatus, error) {
	...
	if l.service() == nil || l.service().Kafka == nil || !l.service().Kafka.Enabled() {
		return dependencyError("kafka", codes.KafkaUnavailable, "Kafka生产者未初始化")
	}
...
func (l *HealthLogic) checkTaskQueue(enabled bool) (types.HealthDependencyStatus, error) {
	...
	if l.service() == nil || l.service().Task == nil || !l.service().Task.IsEnabled() {
		return dependencyError("task_queue", codes.TaskQueueUnavailable, "任务队列未初始化或未启用")
	}
...
func (l *HealthLogic) checkCollector(enabled bool) (types.HealthDependencyStatus, error) {
	...
	if l.service() == nil || l.service().Collector == nil {
		return dependencyError("collector", codes.CollectorUnavailable, "Collector未初始化")
	}
...
func checkSQLDB(ctx context.Context, name string, db *sql.DB, code int) (types.HealthDependencyStatus, error) {
	...
	if err := db.PingContext(checkCtx); err != nil {
		return dependencyError(name, code, fmt.Sprintf("数据库PING失败: %v", err))
	}
...
func dependencyError(name string, code int, message string) (types.HealthDependencyStatus, error) {
	status := types.HealthDependencyStatus{Name: name, Status: healthStatusError, Code: code, Message: message}
	return status, errors.Errorf("ready依赖检查失败 name=%s code=%d message=%s", name, code, message)
}
