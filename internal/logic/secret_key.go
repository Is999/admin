...
const (
	...
	// RSAServerPublicKey 表示服务端 RSA 公钥，用于对外展示或兼容管理。
...
type SecretKeyRouteConfig struct {
	...
	cacheMissingSignStatus   bool   // cacheMissingSignStatus 标记旧缓存是否缺少签名状态字段
	cacheMissingCryptoStatus bool   // cacheMissingCryptoStatus 标记旧缓存是否缺少加密状态字段
}
...
// needsStatusBackfill 判断当前路由缓存是否缺少新增的签名/加密状态字段。
func (c *SecretKeyRouteConfig) needsStatusBackfill() bool {
	return c != nil && (c.cacheMissingSignStatus || c.cacheMissingCryptoStatus)
}
...
// GetAESKeyByAppID 兼容旧调用：按稳定/灰度规则读取 AES KEY 与 IV。
func (l *SecretKeyLogic) GetAESKeyByAppID(appID string) (*AESKey, error) {
	key, _, err := l.GetAESKey(appID, "", "")
	return key, errors.Tag(err)
}
...
// GetRSAKeyByAppID 兼容旧调用：按稳定/灰度规则读取 RSA PEM 内容。
func (l *SecretKeyLogic) GetRSAKeyByAppID(appID string, keyType string) (string, error) {
	text, _, err := l.GetRSAKey(appID, "", "", keyType)
	return text, errors.Tag(err)
}
...
func (l *SecretKeyLogic) getSecretKeyRoute(appID string) (*SecretKeyRouteConfig, error) {
	...
	if err == nil && route != nil && route.needsStatusBackfill() {
		return l.renewSecretKeyRouteCache(appID)
	}
...
func (l *SecretKeyLogic) getSecretKeyRouteCache(appID string) (*SecretKeyRouteConfig, error) {
	...
	data, err := l.Redis().HGetAll(l.Context(), key).Result()
	if err != nil {
		return nil, errors.Tag(err)
	}
	if len(data) == 0 {
	...
	if cacheIsEmptyMarker(data["value"]) {
	...
	return &SecretKeyRouteConfig{
		...
		cacheMissingSignStatus:   !signStatusExists,
		cacheMissingCryptoStatus: !cryptoStatusExists,
	}, nil
}

// parseSecretKeyStatusField 解析路由缓存中的签名/加密开关字段；
// 旧缓存未写入字段时默认兼容为启用，避免发布新字段后旧缓存把链路误判为关闭。
...
func (l *SecretKeyLogic) renewSecretKeyRouteCache(appID string) (*SecretKeyRouteConfig, error) {
	...
	if err := writeDB.Where("uuid = ?", appID).First(&row).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) && l.Redis() != nil {
			...
			pipe.Expire(ctx, routeCacheKey, emptyCacheTTL())
			_, _ = pipe.Exec(ctx)
		}
		return nil, errors.Tag(err)
	}
	...
	pipe.Expire(ctx, routeCacheKey, jitterTTL(time.Hour))
...
func (l *SecretKeyLogic) renewSecretKeyVersionCache(appID string, keyVersion string) (*model.SecretKeyVersion, error) {
	...
	if err := writeDB.Where("uuid = ? AND key_version = ?", appID, keyVersion).First(&row).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) && l.Redis() != nil {
			...
			if secretKeyVersionShouldCacheEmptyMarker(keyVersion, l.secretKeyRouteVersionHints(appID)) {
				...
				pipe.Expire(ctx, aesCacheKey, emptyCacheTTL())
				pipe.Expire(ctx, rsaCacheKey, emptyCacheTTL())
	...
	pipe.Expire(ctx, aesCacheKey, jitterTTL(time.Hour))
	pipe.Expire(ctx, rsaCacheKey, jitterTTL(time.Hour))
...
func deriveServerPublicPEMFromPrivateRef(privateRef string) (string, error) {
	...
	if err != nil {
		return "", errors.Wrap(err, "服务端RSA私钥读取失败")
	}
...
func (l *SecretKeyLogic) secretKeyRoutePhysicalCacheKey(appID string) string {
	return tableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keys.SecretKeyRoute, appID))
}
...
func (l *SecretKeyLogic) secretKeyVersionPhysicalCacheKey(appID string, keyVersion string, typ string) string {
	return tableCachePhysicalKey(l.BaseLogic, secretKeyVersionCacheKey(appID, keyVersion, typ))
}
...
func (l *SecretKeyLogic) secretKeyVersionPhysicalCacheKeys(appID string, keyVersion string) (string, string) {
	...
	return tableCachePhysicalKey(l.BaseLogic, aesCacheKey), tableCachePhysicalKey(l.BaseLogic, rsaCacheKey)
}
...
func (l *SecretKeyLogic) secretKeyVersionPhysicalIndexKey(appID string) string {
	return tableCachePhysicalKey(l.BaseLogic, fmt.Sprintf(keys.SecretKeyVersionIndex, appID))
}

// secretKeyVersionIndexTTL 返回版本材料缓存索引 TTL，索引略长于真实缓存，便于覆盖删除窗口。
func secretKeyVersionIndexTTL() time.Duration {
	return jitterTTL(2 * time.Hour)
}
...
func (l *SecretKeyLogic) deleteSecretKeyVersionCaches(appID string, versionHints ...string) error {
	...
	indexKeys := tableCachePhysicalAndLegacyKeys(l.BaseLogic, fmt.Sprintf(keys.SecretKeyVersionIndex, appID))
	members := make([]string, 0)
	for _, indexKey := range indexKeys {
		currentMembers, err := l.Redis().SMembers(l.Context(), indexKey).Result()
		if err != nil {
			return errors.Tag(err)
		}
	...
	for _, member := range members {
		...
		if member == "" || !secretKeyVersionCacheBelongsToAppID(member, appID) {
			continue
		}
	...
	return errors.Tag(l.RdsDelKeys(tableCachePhysicalAndLegacyKeys(l.BaseLogic, deleteKeys...)...))
}
...
func (l *SecretKeyLogic) secretKeyRouteVersionHints(appID string) []string {
	...
	for _, routeKey := range tableCachePhysicalAndLegacyKeys(l.BaseLogic, fmt.Sprintf(keys.SecretKeyRoute, appID)) {
		data, err := l.Redis().HGetAll(l.Context(), routeKey).Result()
...
func secretKeyVersionCacheBelongsToAppID(cacheKey string, appID string) bool {
	...
	return strings.HasPrefix(cacheKey, fmt.Sprintf(keys.SecretKeyAESVersionPrefix, appID)) ||
		strings.HasPrefix(cacheKey, fmt.Sprintf(keys.SecretKeyRSAVersionPrefix, appID))
}
