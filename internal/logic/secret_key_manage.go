...
const (
	...
	secretKeyRSASignCheckPayload = "admin-cron-sign-check"
)
...
func (l *SecretKeyLogic) logSecretKeySignCheckFailure(uuid string, keyVersion string, stage string, signPayload string, signValue string, secretType string, secretValue string, err error) {
	...
	if err != nil {
		...
		loggerx.Infow(l.Context(), "秘钥 签名自检失败", fields...)
		return
	}
...
func (l *SecretKeyLogic) List(req *types.SecretKeyListReq) *types.BizResult {
	...
	list, total, err := model.List[model.SecretKey](dbq, req.GetPageReq.Page, req.PageSize, orderBy, normalizedOrderDirection(req.Order))
	if err != nil {
...
func (l *SecretKeyLogic) Get(req *types.SecretKeyDetailReq) *types.BizResult {
	if req.ID <= 0 {
		...
		return types.NewBizResult(codes.ParamError).
			...
			WithError(wrapLogicError(err, "SecretKeyLogic.Get 参数校验失败"))
	}
...
func (l *SecretKeyLogic) Create(req *types.SaveSecretKeyReq) *types.BizResult {
	...
	if err := validateSecretKeySaveReq(req, nil); err != nil {
		return types.NewBizResult(codes.ParamError).
			SetI18nMessage(i18n.MsgKeyParamErrorFormat, err.Error()).
			WithError(wrapLogicError(err, "SecretKeyLogic.Create 参数校验失败"))
	}
	now := time.Now()
...
func (l *SecretKeyLogic) Update(req *types.SaveSecretKeyReq) *types.BizResult {
	if req.ID <= 0 {
		...
		return types.NewBizResult(codes.ParamError).
			SetI18nMessage(i18n.MsgKeyParamErrorFormat, err.Error()).
			WithError(wrapLogicError(err, "SecretKeyLogic.Update 参数校验失败"))
	}
	if err := l.requireSecretKeyMFATwoStep(req.TwoStepKey, req.TwoStepValue); err != nil {
		return l.secretKeyMFABizResult(err)
	}
...
func (l *SecretKeyLogic) UpdateStatus(req *types.SecretKeyStatusReq) *types.BizResult {
	...
	if status == 1 {
		if err := validateSecretKeyRouteWithVersions(&types.SaveSecretKeyReq{
			...
			return types.NewBizResult(codes.ParamError).
				SetI18nMessage(i18n.MsgKeyParamErrorFormat, err.Error()).
				WithError(wrapLogicError(err, "SecretKeyLogic.UpdateStatus 参数校验失败"))
		}
...
func (l *SecretKeyLogic) requireSecretKeyMFATwoStep(twoStepKey string, twoStepValue string) error {
	...
	return adminLogic.requireOperateMFATwoStep(MFAScenarioSecretKeyManage, twoStepKey, twoStepValue)
}
...
func (l *SecretKeyLogic) checkSecretKeyPayload(req *types.SaveSecretKeyReq, versions []model.SecretKeyVersion, refreshCache bool, runtimeCheck bool) types.SecretKeyCheckResult {
	...
	if runtimeCheck && result.UUID != "" {
		if cryptoEnabled {
			if aesCipher, err := security.NewAESCipher(aesKeyText, aesIVText); err != nil {
				...
				const aesPlaintext = "admin-cron-secret-check"
		...
		if cryptoEnabled {
			...
			if rsaCipherErr != nil {
				...
				const rsaPlaintext = "admin-cron-rsa-check"
	...
	result.CheckedAt = formatDateTime(time.Now())
...
func runSecretKeyRSASignVerifySelfCheck(serverPrivatePEM string, serverPublicPEM string) (bool, error) {
	const signPayload = "admin-cron-sign-check"
...
func (l *SecretKeyLogic) deleteSecretKeyCache(appID string) error {
	...
	if err := l.RdsDelKeys(tableCachePhysicalAndLegacyKeys(l.BaseLogic, l.secretKeyRoutePhysicalCacheKey(appID))...); err != nil {
		return errors.Tag(err)
	}
	return nil
}
