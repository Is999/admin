...
func TestMaskSecretKeyValue(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "文件路径仅展示文件名摘要",
			input: "/etc/admin-cron/keys/app/server_private.pem",
			want:  "serv****.pem",
		},
		{
			name:  "短文件名保留前缀",
...
func TestSecretKeyModelToItem(t *testing.T) {
	...
	selected := model.SecretKeyVersion{
		ID:                     11,
		KeyVersion:             "v1",
		AESKeyRef:              "/etc/admin-cron/keys/app.demo/aes_key",
		AESIVRef:               "/etc/admin-cron/keys/app.demo/aes_iv",
		RSAPublicKeyUserRef:    "/etc/admin-cron/keys/app.demo/user_public.pem",
		RSAPublicKeyServerRef:  "/etc/admin-cron/keys/app.demo/server_public.pem",
		RSAPrivateKeyServerRef: "/etc/admin-cron/keys/app.demo/server_private.pem",
		Status:                 1,
		Remark:                 "stable",
		CreatedAt:              time.Date(2026, 1, 2, 3, 4, 5, 0, time.Local),
		UpdatedAt:              time.Date(2026, 1, 2, 3, 4, 6, 0, time.Local),
	...
	grayVersion := model.SecretKeyVersion{
		ID:                     12,
		KeyVersion:             "v2",
		AESKeyRef:              "/etc/admin-cron/keys/app.demo/aes_key_v2",
		AESIVRef:               "/etc/admin-cron/keys/app.demo/aes_iv_v2",
		RSAPublicKeyUserRef:    "/etc/admin-cron/keys/app.demo/user_public_v2.pem",
		RSAPublicKeyServerRef:  "/etc/admin-cron/keys/app.demo/server_public_v2.pem",
		RSAPrivateKeyServerRef: "/etc/admin-cron/keys/app.demo/server_private_v2.pem",
		Status:                 1,
		Remark:                 "gray",
