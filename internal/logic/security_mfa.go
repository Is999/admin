...
const (
	...
	// mfaTwoStepSecretSourceCurrent 表示本次 MFA 校验通过的是数据库当前保留的旧秘钥。
...
func (l *SecurityLogic) IssueMFATwoStepTicket(adminID int, scenario int) (*types.UserCompatTwoStepResp, error) {
	return l.issueMFATwoStepTicket(adminID, scenario, nil)
}

// IssueMFATwoStepTicketWithVerifyResult 按指定的秘钥校验结果生成二次校验票据。
func (l *SecurityLogic) IssueMFATwoStepTicketWithVerifyResult(adminID int, scenario int, verifyResult *mfaBindingVerifyResult) (*types.UserCompatTwoStepResp, error) {
	return l.issueMFATwoStepTicket(adminID, scenario, verifyResult)
}

// issueMFATwoStepTicket 生成当前管理员指定场景的二次校验票据，并按需附带绑定秘钥来源元信息。
func (l *SecurityLogic) issueMFATwoStepTicket(adminID int, scenario int, verifyResult *mfaBindingVerifyResult) (*types.UserCompatTwoStepResp, error) {
	if adminID <= 0 {
	...
	return &types.UserCompatTwoStepResp{
		Key:    key,
		Time:   time.Now().Unix(),
...
func (l *SecurityLogic) MFAFrequency() int {
	configCtx, cancel := optionalSecurityConfigContext(l.Context())
	defer cancel()
	value, err := (&SysConfigLogic{BaseLogic: NewBaseLogicWithContext(configCtx, l.svc)}).GetCachedValue(ConfigAdminMFACheckFrequency)
	if err != nil {
		return 0
	}
	switch v := value.(type) {
...
func (l *SecurityLogic) MFADisabledScenarios() []int {
	items := SecurityConfigStringSlice(l.Context(), l.svc, ConfigAdminDisableMFACheckScenario, nil)
...
func (l *SecurityLogic) NeedMFATwoStep(admin *model.Admin, scenario int) bool {
	...
	forceMFA := SecurityConfigBool(l.Context(), l.svc, ConfigAdminMFACheckEnable, false)
...
func buildAdminMFAURLBySecret(admin *model.Admin, secret string) (string, error) {
	...
	issuer := sanitizeMFALabelValue(mfaDefaultIssuer)
	if issuer == "" {
		issuer = "admin-cron"
	}
	secret = normalizeMFASecret(secret)
...
func (l *SecurityLogic) BuildAdminMFAURL(admin *model.Admin) (string, error) {
	...
	if isUsableMFASecret(currentSecret) {
		return buildAdminMFAURLBySecret(admin, currentSecret)
	}
...
func (l *SecurityLogic) BuildFreshAdminMFAURL(admin *model.Admin) (string, error) {
	...
	return buildAdminMFAURLBySecret(admin, secret)
}

// buildMFAOtpauthURL 按更稳妥的微软兼容格式拼装 otpauth 地址。
...
// loadAdminMFASecret 读取管理员当前可用的 MFA 明文秘钥；数据异常时按未绑定处理，避免旧值继续泄露。
func (l *SecurityLogic) loadAdminMFASecret(admin *model.Admin) (string, error) {
	...
	if err != nil {
		logWrappedError(l, err, "SecurityLogic.loadAdminMFASecret 解析管理员ID[%d]MFA秘钥失败", admin.ID)
		return "", nil
	}
	return secret, nil
}
...
// decodeMFATwoStepTicketPayload 解析 Redis 中缓存的 MFA 二次票据内容，兼容旧格式 `场景:值`。
...
// generateMFASecret 使用加密安全随机数生成 16 位 Base32 秘钥，兼容前端当前输入限制。
