...
// TestBuildAdminMFAURLUsesMicrosoftCompatibleFormat 验证 MFA 绑定地址会对 label 做编码，并固定微软兼容的参数顺序。
func TestBuildAdminMFAURLUsesMicrosoftCompatibleFormat(t *testing.T) {
	logicObj := newTestSecurityLogic()
	...
	want := "otpauth://totp/admin-cron%3Aadmin999?secret=RCABDVITFNQJJ4VJ&issuer=admin-cron&algorithm=SHA1&digits=6&period=30"
	if got != want {
...
// TestVerifyBindingMFACodeFallsBackToCurrentSecret 验证新秘钥失败后回退旧秘钥。
