...
type SecurityDebugLogic struct {
	*corelogic.BaseLogic // 复用上下文、日志、审计和秘钥读取能力
}
...
func (l *SecurityDebugLogic) Sign(req *types.SecurityDebugSignReq) *types.BizResult {
	...
	signText := security.BuildSignString(payload, signFields, traceID, timestamp, req.AppID)
...
func (l *SecurityDebugLogic) buildSigner(appID string, signatureType string, isVerify bool) (security.Signer, error) {
	...
	case security.SignatureTypeRSA:
		...
		pemText, _, err := secretkeylogic.NewSecretKeyLogic(l.Ctx, l.Svc).GetRSAKey(appID, "", "", keyType)
		if err != nil {
			return nil, errors.Tag(err)
		}
		if isVerify {
...
// normalizedDebugTimestamp 生成调试用秒级时间戳，保持和真实签名链路一致。
func normalizedDebugTimestamp(timestamp string) string {
