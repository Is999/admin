...
func (l *SysConfigLogic) ExportExcel(req *types.SysConfigExcelExportReq) (string, string, *types.BizResult) {
	...
	err := redislock.WithLock(l.Context(), l.Redis(), lockKey, sysConfigExcelLockTTL, func(ctx context.Context) error {
		...
		exportPath = filepath.Join(os.TempDir(), "admin-cron", "exports", "sys-config", fileName)
...
func (l *SysConfigLogic) ImportExcel(req *types.SysConfigExcelImportReq) *types.BizResult {
	fileTransferLogic := NewFileTransferLogicWithContext(l.Context(), l.svc)
	...
	for uuid := range changedUUIDs {
		_ = l.RdsDelKeys(tableCachePhysicalAndLegacyKeys(l.BaseLogic, fmt.Sprintf(keys.SysConfigUUID, uuid))...)
...
func (l *SysConfigLogic) resolveImportExcelFile(req *types.SysConfigExcelImportReq, fileTransferLogic *FileTransferLogic) (string, func(), error) {
	if req == nil {
	...
	if strings.TrimSpace(req.UploadID) != "" {
		...
		if strings.TrimSpace(session.BizType) != FileTransferBizSysConfigExcelImport {
