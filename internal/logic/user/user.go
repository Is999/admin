...
// listByUserIdentity 通过身份目录分页并按当前路由批量读取用户表。
func (l *Logic) listByUserIdentity(db *gorm.DB, req *types.UserListReq) *types.BizResult {
	...
	// 身份目录排序使用独立会话，避免 user_id 子句泄漏到后续以 id 为主键的用户表查询。
	dbq := db.Session(&gorm.Session{}).Model(&model.UserIdentity{}).Table(tableName)
	if req.ID > 0 {
	...
	users, err := model.FindUsersByIdentityRows(db, identities, l.Svc.CurrentConfig().User.RouteShardCount)
	if err != nil {
	...
	for index := range identities {
		items = append(items, userModelToItem(users[identities[index].UserID]))
	}
	nextCursorID := int64(0)
	if hasMore && len(identities) > 0 {
		nextCursorID = identities[len(identities)-1].UserID
	}
	// 身份目录分页用已浏览行数和下一页占位驱动分页控件，不对分片用户表执行实时 COUNT。
...
func (l *Logic) Update(req *types.UpdateUserReq) *types.BizResult {
	...
	if len(updates) > 0 {
		...
		if err = model.UpdateUserProfileWithIdentities(db, req.ID, updates, cfg.AppKey, cfg.User.RouteShardCount); err != nil {
...
func (l *Logic) UpdateStatus(req *types.UserStatusReq) *types.BizResult {
	...
	if row.Status != status {
		...
		now := time.Now()
		authVersion, err := model.UpdateUserStatusAndAuthVersion(db, req.ID, status, now, l.Svc.CurrentConfig().User.RouteShardCount)
		if err != nil {
...
func (l *Logic) ResetPassword(req *types.ResetUserPasswordReq) *types.BizResult {
	...
	now := time.Now()
	authVersion, err := model.UpdateUserPasswordAndAuthVersion(db, req.ID, string(passwordHash), now, l.Svc.CurrentConfig().User.RouteShardCount)
	if err != nil {
...
func (l *Logic) SyncRuntime(req *types.SyncUserRuntimeReq) *types.BizResult {
	...
	if req.Sessions {
		...
		authVersion, err = model.BumpUserAuthVersion(db, req.ID, time.Now(), l.Svc.CurrentConfig().User.RouteShardCount)
		if err != nil {
...
func (l *Logic) getUser(id int64) (*model.User, error) {
	...
	row, err := model.FindUserByID(db, id, l.Svc.CurrentConfig().User.RouteShardCount)
	if err != nil {
		return nil, errors.Tag(err)
	}
	if row == nil {
