...
const (
	...
	opsNonceBytes = 16
	// opsSignatureBodyMaxBytes 限制参与验签的请求体大小。
...
// OpsMiddleware 保护管理员初始化和内部任务触发等运维级接口。
...
func (m *OpsMiddleware) Handle(next http.HandlerFunc, alias RouteAlias) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		auth, err := authenticateOpsWithClientIP(r, cfg, clientIP, time.Now())
		if err != nil {
...
func authenticateOpsWithClientIP(r *http.Request, cfg config.OpsConfig, clientIP string, now time.Time) (opsRequestAuth, error) {
	...
	if !clientIPAllowed(clientIP, cfg.AllowedIPs) {
