...
// TestSignatureMiddlewarePassesThroughRequestSignOnlyResponse 确保仅请求验签的下载响应不被整包缓冲。
func TestSignatureMiddlewarePassesThroughRequestSignOnlyResponse(t *testing.T) {
	server := miniredis.RunT(t)
	client := redis.NewClient(&redis.Options{Addr: server.Addr()})
	t.Cleanup(func() {
		_ = client.Close()
	})
	cfg := securityMiddlewareConfig(1, 0)
	prev := runtimecfg.Get()
	runtimecfg.Set(cfg)
	t.Cleanup(func() {
		runtimecfg.Restore(prev)
	...
	traceID := "request-sign-stream-trace"
	timestamp := strconv.FormatInt(time.Now().Unix(), 10)
	params := map[string]any{"backupId": "backup-1"}
	cipherObj, err := security.NewAESCipher(cfg.Security.SecretKey.AESKey, cfg.Security.SecretKey.AESIV)
	if err != nil {
		t.Fatalf("NewAESCipher() error = %v", err)
	}
	sign, err := cipherObj.Sign(security.BuildSignString(
		params,
		security.PolicyByRoute(string(routealias.SysConfigImport)).RequestSign,
		traceID,
		timestamp,
		cfg.AppID,
	))
	...
	preservedFlusher := false
	handler := NewSignatureMiddleware(svc.NewServiceContext(cfg, svc.Dependencies{Rds: client})).Handle(
		func(w http.ResponseWriter, _ *http.Request) {
			_, preservedFlusher = w.(http.Flusher)
	...
	request := httptest.NewRequest(
		http.MethodGet,
		"/api/dicts/import/backup/backup-1?backupId=backup-1&sign="+url.QueryEscape(sign),
		nil,
	)
