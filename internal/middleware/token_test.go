...
func TestVerifyAdminTokenRequiresCachedSession(t *testing.T) {
	...
	if err := cachelogic.NewCacheLogic(ctx, svcCtx).SetAdminSession(1, &types.AdminSession{
		ID:       1,
		UserName: "admin",
		Token:    token,
	}); err != nil {
	...
	if identity.Session == nil || identity.Session.ID != 1 || identity.Session.Token != token {
		t.Fatalf("解析出的会话信息不符合预期: %+v", identity.Session)
	}
}
...
func TestVerifyAdminTokenDoesNotDeleteCurrentSessionForExpiredOldToken(t *testing.T) {
	...
	if err := cacheLogic.SetAdminSession(1, &types.AdminSession{ID: 1, UserName: "admin", Token: currentToken}); err != nil {
...
func TestRefreshTokenGraceIsLimitedToSessionFinalizationRoutes(t *testing.T) {
	...
	cacheLogic := cachelogic.NewCacheLogic(context.Background(), svcCtx)
	if err := cacheLogic.SetAdminSession(1, &types.AdminSession{ID: 1, UserName: "admin", Token: oldToken}); err != nil {
...
// TestVerifyAdminTokenRejectsMissingRedisSessionStore 校验需要会话态时，未注入 Redis 会返回依赖故障而不是伪装成无效 token。
func TestVerifyAdminTokenRejectsMissingRedisSessionStore(t *testing.T) {
	...
	if !errors.Is(err, cachelogic.ErrRedisUnavailable) {
		t.Fatalf("期望返回 ErrRedisUnavailable，实际为 %v", err)
	}
}

// TestVerifyAdminTokenReportsRedisCommandFailure 验证 Redis 命令异常不会被误报为 token 无效。
func TestVerifyAdminTokenReportsRedisCommandFailure(t *testing.T) {
	server := miniredis.RunT(t)
	server.Server().SetPreHook(func(peer *miniredisserver.Peer, command string, _ ...string) bool {
		if !strings.EqualFold(command, "hgetall") {
			return false
		}
	...
	const secret = "test-secret"
	token := testAdminToken(t, secret, 1, "admin", time.Now().Add(time.Hour))
	svcCtx := svc.NewServiceContext(config.Config{JwtSecret: secret}, svc.Dependencies{Rds: client})
