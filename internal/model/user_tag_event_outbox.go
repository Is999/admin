...
type UserTagEventOutbox struct {
	...
	ShardNo      int                     `gorm:"column:shard_no;type:int;not null;default:0;index:idx_workflow_state_shard_id,priority:3;comment:CRC32(uid字符串)%1024固定逻辑桶" json:"shard_no"`                                                     // 与 user.shard_no 一致的固定逻辑桶
