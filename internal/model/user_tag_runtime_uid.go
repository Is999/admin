...
type UserTagRuntimeUID struct {
	...
	UID        int64     `gorm:"column:uid;type:bigint;primaryKey;index:idx_workflow_shard_uid,priority:3;index:idx_created_uid,priority:2;comment:用户 ID" json:"uid"` // 用户 ID
	ShardNo    int       `gorm:"column:shard_no;type:int;not null;default:0;index:idx_workflow_shard_uid,priority:2;comment:CRC32(uid字符串)%1024固定逻辑桶" json:"shard_no"` // 与 user.shard_no 一致的固定逻辑桶
