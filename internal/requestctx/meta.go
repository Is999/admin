...
type Meta struct {
	...
	EnabledRoleIDs         []int     // 当前请求已解析的启用角色 ID，按升序去重
	EnabledRolesResolved   bool      // 是否已解析启用角色，空结果也避免重复读取缓存
}
...
// SetEnabledRoleIDs 保存当前请求已解析的启用角色 ID。
func SetEnabledRoleIDs(ctx context.Context, roleIDs []int) {
	if meta := FromContext(ctx); meta != nil {
		meta.EnabledRoleIDs = append(meta.EnabledRoleIDs[:0], roleIDs...)
		meta.EnabledRolesResolved = true
	}
}

// EnabledRoleIDs 返回当前请求已解析的启用角色 ID。
func EnabledRoleIDs(ctx context.Context) ([]int, bool) {
	meta := FromContext(ctx)
	if meta == nil || !meta.EnabledRolesResolved {
		return nil, false
	}
	return append([]int(nil), meta.EnabledRoleIDs...), true
}
