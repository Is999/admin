-- 代码资产：仅允许当前 attempt 更新任务运行快照，防止租约丢失后迟到实例覆盖新实例。
-- 调用方：internal/task/queue.Manager.writeTaskRuntimeAttempt；go:embed 加载后由 embedasset 剥离本说明再发送 Redis。
-- KEYS[1]：按队列和 taskID 隔离的运行快照 hash。
-- ARGV[1]：当前 attempt token。
-- ARGV[2]：运行快照 TTL，毫秒。
-- ARGV[3]：本次写入前需清理的字段数量。
