...
func (m *Manager) traceAndLogMiddleware() asynq.MiddlewareFunc {
	return func(next asynq.Handler) asynq.Handler {
		return asynq.HandlerFunc(func(ctx context.Context, task *asynq.Task) error {
			...
			ctx = m.enrichTaskContext(ctx, task)
			ctx, statsTracker := taskstats.WithTracker(ctx, taskNameFromTask(task))
			...
			stopRuntimeHeartbeat := m.startTaskRuntimeHeartbeat(
				ctx,
				queue,
				taskID,
				attemptToken,
			...
			if err == nil && !skipBusiness {
				if statsStartErr := m.startWorkflowTaskStatsAttempt(ctx, workflowMeta, attemptToken); statsStartErr != nil {
					loggerx.Errorw(ctx, "工作流节点处理量 attempt 初始化失败", statsStartErr, taskLogFields(task)...)
				}
				businessRan = true
			...
			persistedStatsSnapshot := boundedTaskStatsSnapshot(statsSnapshot)
			statsWriteCtx, statsWriteCancel := m.taskFinalWriteContext(ctx)
			statsErr := m.recordWorkflowTaskStats(statsWriteCtx, workflowMeta, attemptToken, persistedStatsSnapshot)
			statsWriteCancel()
			if statsErr != nil {
			...
			if err == nil {
				...
				if markErr := m.markTaskSuccess(ctx, workflowMeta); markErr != nil {
					err = errors.Join(ErrWorkflowDispatch, markErr)
				}
			}
			m.recordTaskRuntimeFinish(ctx, queue, taskID, attemptToken, begin, err, persistedStatsSnapshot)
