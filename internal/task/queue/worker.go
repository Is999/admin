...
func (m *Manager) StartScheduler() error {
	...
	leaseKey := m.schedulerLeaseKey()
	if leaseKey == "" {
		return errors.Errorf("任务调度 leader 租约键无效 app_id=%s", m.appNamespace())
	}
	...
	m.leader = NewLeaderRunner(m.redis, leaseKey, m.schedulerLeaseTTL(), m.schedulerRenewInterval(), func(leaderCtx context.Context) (func(), error) {
