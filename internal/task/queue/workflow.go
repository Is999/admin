...
const (
	...
	// workflowNodeTraceAttemptFieldPrefix 是节点 hash 内部的分片处理量 attempt 字段前缀。
	workflowNodeTraceAttemptFieldPrefix = "__execution_trace_attempt:"
...
// workflowNodeTraceAttemptField 返回节点 hash 中单个分片的处理量 attempt 字段名。
func workflowNodeTraceAttemptField(shardIndex int) string {
	return fmt.Sprintf("%s%d", workflowNodeTraceAttemptFieldPrefix, shardIndex)
}
...
type WorkflowDefinition struct {
	...
	MaxShardTotal            int                                // 工作流自定义分片上限，0 表示使用系统上限
...
// startWorkflowTaskStatsAttempt 初始化分片处理量 attempt，并清理上一轮重试快照。
func (m *Manager) startWorkflowTaskStatsAttempt(ctx context.Context, meta WorkflowTaskMeta, attemptToken string) error {
	attemptToken = strings.TrimSpace(attemptToken)
	if m == nil || m.redis == nil || attemptToken == "" || strings.TrimSpace(meta.WorkflowID) == "" || strings.TrimSpace(meta.WorkflowNode) == "" {
		return nil
	}
	nodeKey := m.workflowNodeKey(meta.WorkflowID, meta.WorkflowNode)
	_, err := m.redis.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
		pipe.HSet(ctx, nodeKey, workflowNodeTraceAttemptField(meta.ShardIndex), attemptToken)
		pipe.HDel(ctx, nodeKey, workflowNodeTraceField(meta.ShardIndex))
		return nil
	})
	if err != nil {
		return errors.Wrapf(err, "初始化工作流节点处理量 attempt 失败 workflow_id=%s node=%s shard=%d", meta.WorkflowID, meta.WorkflowNode, meta.ShardIndex)
	}
	return nil
}

// recordWorkflowTaskStats 仅由当前 attempt 保存分片处理量快照，供工作流状态页聚合展示。
func (m *Manager) recordWorkflowTaskStats(ctx context.Context, meta WorkflowTaskMeta, attemptToken string, snapshot *taskstats.Snapshot) error {
	attemptToken = strings.TrimSpace(attemptToken)
	if m == nil || m.redis == nil || attemptToken == "" || snapshot == nil || snapshot.Empty() || strings.TrimSpace(meta.WorkflowID) == "" || strings.TrimSpace(meta.WorkflowNode) == "" {
		return nil
	}
	...
	result, err := writeWorkflowTaskStatsAttemptScript.Run(
		ctx,
		m.redis,
		[]string{m.workflowNodeKey(meta.WorkflowID, meta.WorkflowNode)},
		workflowNodeTraceAttemptField(meta.ShardIndex),
		attemptToken,
		workflowNodeTraceField(meta.ShardIndex),
		string(raw),
	).Int()
	if err != nil {
		return errors.Wrapf(err, "原子记录工作流节点处理量失败 workflow_id=%s node=%s shard=%d", meta.WorkflowID, meta.WorkflowNode, meta.ShardIndex)
	}
	if result != 0 && result != 1 {
		return errors.Errorf("工作流节点处理量脚本返回非法结果 workflow_id=%s node=%s shard=%d result=%d", meta.WorkflowID, meta.WorkflowNode, meta.ShardIndex, result)
	}
	return nil
}
...
func validateWorkflowDefinition(def *WorkflowDefinition) error {
	...
	if def.MaxShardTotal < 0 || def.MaxShardTotal > tasklimits.MaxShardTotal {
		return errors.Errorf("工作流 %s 的最大分片数必须在 0-%d 之间", def.Name, tasklimits.MaxShardTotal)
	}
	if len(def.Nodes) == 0 {
...
func (m *Manager) readWorkflowStatus(ctx context.Context, workflowID string) (*WorkflowMetaRecord, []WorkflowNodeRecord, error) {
	...
	nodeCommands := make(map[string]*redis.MapStringStringCmd, len(nodeNames))
	pipe := m.redis.Pipeline()
	for _, nodeName := range nodeNames {
		nodeCommands[nodeName] = pipe.HGetAll(ctx, m.workflowNodeKey(workflowID, nodeName))
	}
	if _, execErr := pipe.Exec(ctx); execErr != nil {
		pipe.Discard()
		return nil, nil, errors.Wrapf(execErr, "批量读取工作流节点失败 workflow_id=%s", workflowID)
	}
	pipe.Discard()
	nodes := make([]WorkflowNodeRecord, 0, len(nodeNames))
	for _, nodeName := range nodeNames {
		vals, readErr := nodeCommands[nodeName].Result()
		if readErr != nil {
