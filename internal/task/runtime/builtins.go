...
// 该方法供测试和显式装配入口复用；启动默认清单以 bootstrap.defaultTaskPlugins 为唯一来源。
func BuiltinPlugins() []Plugin {
	return []Plugin{
		NewCorePlugin(),
		NewPluginFunc(archivetask.PluginName, func(runtime *Runtime) error {
			return archivetask.Setup(runtime)
		}),
		NewPluginFunc(exportjob.PluginName, func(runtime *Runtime) error {
			return exportjob.Setup(runtime)
		}),
		NewPluginFunc(usertagtask.PluginName, func(runtime *Runtime) error {
