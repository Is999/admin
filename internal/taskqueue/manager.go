...
const (
	...
	asynqRedisKeyPrefix = "asynq" // Asynq 默认 Redis key 前缀
)
...
func New(cfg config.TaskQueueConfig, client redis.UniversalClient) *Manager {
	...
	m := &Manager{
		...
		logger:     asynqLogger{},
		tracer:     otel.Tracer("admin_cron/task"),
...
func (m *Manager) appNamespace() string {
	...
	return strings.TrimSpace(m.CurrentConfig().AppID)
}
...
func (m *Manager) namespacedQueueName(queue string) string {
	...
	namespace := m.appNamespace()
	if namespace == "" {
		return queue
	}
	return fmt.Sprintf(keys.TaskQueueNamespacedName, namespace, queue)
}
...
func (m *Manager) displayQueueName(queue string) string {
	...
	prefix := fmt.Sprintf(keys.TaskQueueNamespacePrefix, namespace)
	return strings.TrimPrefix(queue, prefix)
}

// redisKeyPrefix 返回当前站点在 Redis 中使用的任务 key 命名空间前缀。
func (m *Manager) redisKeyPrefix() string {
	namespace := m.appNamespace()
	if namespace == "" {
		return keys.TaskQueueDefaultRedisPrefix
	}
	return fmt.Sprintf(keys.TaskQueueAppRedisPrefix, namespace)
}

// namespacedRedisKey 返回当前站点下的完整 Redis key。
func (m *Manager) namespacedRedisKey(parts ...string) string {
	allParts := []string{m.redisKeyPrefix()}
...
func (m *Manager) workflowKeyPrefix() string {
	return m.namespacedRedisKey("workflow")
}
...
func (m *Manager) namespacedGroup(group string) string {
	...
	return fmt.Sprintf(keys.TaskQueueNamespacedName, namespace, group)
}
...
func (m *Manager) namespacedUniqueKey(key string) string {
	...
	return fmt.Sprintf(keys.TaskQueueNamespacedName, namespace, key)
}
...
func (m *Manager) taskRuntimeKey(taskID string) string {
	return m.namespacedRedisKey("task-runtime", strings.TrimSpace(taskID))
}
...
func (m *Manager) expireFinalTaskRecord(ctx context.Context, queue string, taskID string, runErr error) {
	...
	defer cancel()
	if err := m.redis.Expire(writeCtx, asynqTaskHashKey(queue, taskID), retention+taskRecordTTLBuffer).Err(); err != nil {
...
func workflowTargetsExample(name string) string {
	switch name {
	case WorkflowNameCacheRefresh:
		return "admin_profile:1, admin_roles_detail:1"
	case archive.WorkflowNameRun:
		return "admin_log"
	case "user_tag.targeted.calc":
...
func (m *Manager) deleteExpiredArchivedTasks(ctx context.Context, internalQueue string, cutoffUnix int64, batchSize int) (int64, error) {
	...
	archivedKey, err := asynqTaskZSetKey(internalQueue, "archived")
	if err != nil {
		return 0, errors.Tag(err)
	}
	result, err := deleteExpiredArchivedTasksScript.Run(ctx, m.redis, []string{archivedKey},
		cutoffUnix,
		asynqTaskHashKeyPrefix(internalQueue),
		batchSize,
	).Result()
	if err != nil {
		return 0, errors.Tag(err)
...
func (m *Manager) visibleServerQueues(queues map[string]int) (map[string]int, bool) {
	...
	for queueName, weight := range queues {
		...
		result[strings.TrimPrefix(queueName, prefix)] = weight
	}
	return result, len(result) > 0
}
...
func (m *Manager) visibleQueueNames(queueNames []string) []string {
	...
	for _, queueName := range queueNames {
		...
		result = append(result, strings.TrimPrefix(queueName, prefix))
	}
	if len(result) > 0 {
		return result
	}
...
func (m *Manager) listDescZSetTaskInfoPage(ctx context.Context, internalQueue string, state string, page int, pageSize int, timeRange taskListTimeRange) ([]*asynq.TaskInfo, int64, error) {
	...
	key, err := asynqTaskZSetKey(internalQueue, state)
	if err != nil {
		return nil, 0, errors.Tag(err)
	}
...
func (m *Manager) scheduledTaskTotal(ctx context.Context, internalQueue string, timeRange taskListTimeRange) (int64, error) {
	...
	total, err := m.redis.ZCount(ctx, asynqScheduledTasksKey(internalQueue), minScore, maxScore).Result()
	if err != nil {
		return 0, errors.Tag(err)
	}
	return total, nil
...
func (m *Manager) listScheduledTaskIDs(ctx context.Context, internalQueue string, page int, pageSize int, timeRange taskListTimeRange) ([]string, error) {
	pageStart, pageStop := taskListPageRange(page, pageSize)
	key := asynqScheduledTasksKey(internalQueue)
	if !timeRange.hasRange {
...
// asynqScheduledTasksKey 返回 Asynq scheduled zset 的 Redis key。
func asynqScheduledTasksKey(queue string) string {
	return fmt.Sprintf("%s:{%s}:scheduled", asynqRedisKeyPrefix, queue)
}

// asynqTaskHashKeyPrefix 返回 Asynq 任务详情 hash 的 Redis key 前缀。
func asynqTaskHashKeyPrefix(queue string) string {
	return fmt.Sprintf("%s:{%s}:t:", asynqRedisKeyPrefix, strings.TrimSpace(queue))
}

// asynqTaskHashKey 返回 Asynq 单个任务详情 hash 的 Redis key。
func asynqTaskHashKey(queue string, taskID string) string {
	return asynqTaskHashKeyPrefix(queue) + strings.TrimSpace(taskID)
}

// asynqTaskZSetKey 返回 Asynq zset 状态对应的 Redis key。
func asynqTaskZSetKey(queue string, state string) (string, error) {
	switch strings.TrimSpace(state) {
	case "retry":
		return fmt.Sprintf("%s:{%s}:retry", asynqRedisKeyPrefix, queue), nil
	case "archived":
		return fmt.Sprintf("%s:{%s}:archived", asynqRedisKeyPrefix, queue), nil
	case "completed":
		return fmt.Sprintf("%s:{%s}:completed", asynqRedisKeyPrefix, queue), nil
	default:
...
func (m *Manager) persistTaskRecordForRerun(ctx context.Context, queue string, taskID string) error {
	...
	writeCtx, cancel := m.taskFinalWriteContext(ctx)
	defer cancel()
	return errors.Tag(m.redis.Persist(writeCtx, asynqTaskHashKey(queue, taskID)).Err())
}
...
func (m *Manager) periodicConfigs() ([]*asynq.PeriodicTaskConfig, error) {
	...
	for idx, item := range items {
		item, invalidReason := m.validatePeriodicTaskConfig(item)
		if invalidReason != "" {
			notifyPeriodicTaskConfigInvalid(idx, item, invalidReason)
			continue
		}
		payload := WorkflowTriggerPayload{
...
func (m *Manager) ValidatePeriodicTaskDefinitions() error {
	...
	for idx, item := range items {
		...
		notifyPeriodicTaskConfigInvalid(idx, normalized, invalidReason)
	}
	return nil
}

// validatePeriodicTaskConfig 校验并归一化单个周期任务配置，异常时仅跳过当前任务。
func (m *Manager) validatePeriodicTaskConfig(item config.TaskPeriodicConfig) (config.TaskPeriodicConfig, string) {
	...
	if item.Workflow == "" {
		return item, "周期任务 workflow 不能为空"
	}
	...
	if item.ShardTotal < 0 {
		return item, "周期任务 shard_total 不能小于 0"
	}
	if item.GrayPercent < 0 || item.GrayPercent > 100 {
		return item, "周期任务 gray_percent 必须在 0 到 100 之间"
	}
	if item.Retry < 0 {
		return item, "周期任务 retry 不能小于 0"
	}
	if item.TimeoutSeconds < 0 {
		return item, "周期任务 timeout_seconds 不能小于 0"
	}
	if item.UniqueTTLSeconds < 0 {
		return item, "周期任务 unique_ttl_seconds 不能小于 0"
	}
	if item.Deadline != "" {
		if _, err = time.Parse(time.RFC3339, item.Deadline); err != nil {
			return item, errors.Wrap(err, "解析周期任务 deadline 失败").Error()
		}
	}
	if item.UniqueKey != "" && item.UniqueTTLSeconds == 0 {
...
func (m *Manager) schedulerLeaseKeyByConfig(cfg config.TaskQueueConfig) string {
	baseKey := helper.FirstNonEmptyString(strings.TrimSpace(cfg.Scheduler.LeaseKey), keys.TaskQueueSchedulerLeaderDefault)
	namespace := strings.TrimSpace(cfg.AppID)
	if namespace == "" {
		return keys.TaskQueueDefaultRedisPrefix + ":" + strings.TrimPrefix(baseKey, keys.TaskQueueDefaultRedisPrefix+":")
	}
	return fmt.Sprintf(keys.TaskQueueAppRedisPrefix, namespace) + ":" + strings.TrimPrefix(baseKey, keys.TaskQueueDefaultRedisPrefix+":")
}
