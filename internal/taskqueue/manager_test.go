...
func TestManagerUpdateConfigRefreshesSnapshot(t *testing.T) {
	...
	if got := manager.schedulerLeaseKey(); got != "task:scheduler:new" {
...
func TestManagerNamespacesTaskArtifactsByAppID(t *testing.T) {
	...
	if got := manager.schedulerLeaseKey(); got != "task:app:215:scheduler:leader" {
		t.Fatalf("期望调度租约键带站点命名空间，实际为 %q", got)
	}
	if got := manager.workflowMetaKey("wf-1"); got != "task:app:215:workflow:wf-1:meta" {
		t.Fatalf("期望工作流元数据键带站点命名空间，实际为 %q", got)
	}
	if got := manager.workflowUniqueKey("user_tag.delta.refresh", "daily"); got != "task:app:215:workflow:unique:user_tag.delta.refresh:daily" {
...
func TestWorkflowShardedSuccessDedupesDuplicateShardAck(t *testing.T) {
	...
	legacyDoneKeys, err := manager.redis.Exists(context.Background(), manager.workflowInstanceKey(workflowID, "root", 0)).Result()
	if err != nil {
		t.Fatalf("读取旧分片实例去重 key 失败: %v", err)
	}
	if legacyDoneKeys != 0 {
		t.Fatalf("期望不再创建旧分片实例去重 key，实际数量为 %d", legacyDoneKeys)
	}
	if err := manager.markTaskSuccess(context.Background(), successMeta); err != nil {
...
func TestTaskRetryMovesTaskToRetryQueue(t *testing.T) {
	...
	waitForCondition(t, 5*time.Second, func() bool {
		info, err := manager.inspector.GetQueueInfo(QueueDefault)
...
func TestTaskRetryDoesNotExpireActiveCache(t *testing.T) {
	...
	waitForCondition(t, 5*time.Second, func() bool {
		info, infoErr := manager.inspector.GetTaskInfo(QueueDefault, resp.TaskID)
		return infoErr == nil && info.State == asynq.TaskStateRetry
	})

	ttl := manager.redis.TTL(context.Background(), asynqTaskHashKey(QueueDefault, resp.TaskID)).Val()
	if ttl != -1 {
...
func TestFinalFailureHookOnlyRunsForArchivedTask(t *testing.T) {
	...
	waitForCondition(t, 5*time.Second, func() bool {
		info, infoErr := manager.inspector.GetQueueInfo(QueueDefault)
...
func TestCompletedTaskTTLUsesNamespacedQueue(t *testing.T) {
	...
	internalKey := asynqTaskHashKey(internalQueue, resp.TaskID)
	...
	if exists := manager.redis.Exists(context.Background(), asynqTaskHashKey(QueueDefault, resp.TaskID)).Val(); exists != 0 {
...
func TestRegisterGroupAggregatorDispatchesByGroup(t *testing.T) {
	...
	got := manager.aggregateTasks("demo.group", []*asynq.Task{
...
func TestRunTaskMovesScheduledTaskToPending(t *testing.T) {
	...
	waitForCondition(t, 3*time.Second, func() bool {
		info, err := manager.inspector.GetTaskInfo(QueueDefault, resp.TaskID)
...
func TestDeleteTaskRemovesScheduledTask(t *testing.T) {
	...
	if _, err := manager.inspector.GetTaskInfo(QueueDefault, resp.TaskID); !errors.Is(err, asynq.ErrTaskNotFound) {
...
func TestExpireFinalTaskRecordSetsTTL(t *testing.T) {
	...
	completedKey := asynqTaskHashKey(internalQueue, completedTaskID)
	...
	archivedKey := asynqTaskHashKey(internalQueue, archivedTaskID)
...
func TestDeleteExpiredArchivedTasksRemovesTaskCache(t *testing.T) {
	...
	uniqueKey := "asynq:{app:1:maintenance}:unique:test"
	newTaskKey := asynqTaskHashKey(internalQueue, newTaskID)
	...
	for index, oldTaskID := range oldTaskIDs {
		...
		if err = manager.redis.HSet(ctx, asynqTaskHashKey(internalQueue, oldTaskID), fields...).Err(); err != nil {
	...
	for _, oldTaskID := range oldTaskIDs {
		oldTaskKeys = append(oldTaskKeys, asynqTaskHashKey(internalQueue, oldTaskID))
	}
