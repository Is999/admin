...
// Validate 校验密码校验请求。
func (r *ProfileCheckSecureReq) Validate() error {
...
// ProfileCheckMFAReq 表示校验 MFA 动态码的请求。
type ProfileCheckMFAReq struct {
...
func (r *ProfileCheckMFAReq) Secret() string {
	return strings.ToUpper(strings.TrimSpace(r.MfaSecureKey))
}

// Validate 校验 MFA 动态码请求。
func (r *ProfileCheckMFAReq) Validate() error {
	if strings.TrimSpace(r.Secure) == "" {
...
// ProfilePasswordReq 表示个人中心修改密码请求。
...
// Validate 校验修改密码请求。
func (r *ProfilePasswordReq) Validate() error {
...
// ProfileUpdateReq 表示个人中心更新基础资料请求。
...
// Validate 校验个人中心更新请求。
func (r *ProfileUpdateReq) Validate() error {
...
// ProfileMFAStatusReq 表示个人中心修改 MFA 状态请求。
type ProfileMFAStatusReq struct {
...
func (r *ProfileMFAStatusReq) Status() int {
	if r.MfaStatus != nil {
		return *r.MfaStatus
	}
	return -1
...
// Secret 返回归一化后的 MFA 秘钥。
func (r *ProfileMFAStatusReq) Secret() string {
	return strings.ToUpper(strings.TrimSpace(r.MfaSecureKey))
}

// Validate 校验修改 MFA 状态请求。
func (r *ProfileMFAStatusReq) Validate() error {
	status := r.Status()
...
// ProfileMFASecretReq 表示个人中心修改 MFA 秘钥请求。
type ProfileMFASecretReq struct {
...
// Secret 返回归一化后的 MFA 秘钥。
func (r *ProfileMFASecretReq) Secret() string {
	return strings.ToUpper(strings.TrimSpace(r.MfaSecureKey))
}

// Validate 校验修改 MFA 秘钥请求。
func (r *ProfileMFASecretReq) Validate() error {
	if len(r.Secret()) != 16 {
		return errors.Errorf("MFA秘钥长度必须是16位")
	}
	return nil
...
// ProfileMFASecretRefreshReq 表示个人中心重新生成 MFA 秘钥请求。
type ProfileMFASecretRefreshReq struct {
	TwoStepKey   string `json:"twoStepKey,optional"`   // 二次校验票据 key
	TwoStepValue string `json:"twoStepValue,optional"` // 二次校验票据 value
}

// Validate 校验重新生成 MFA 秘钥请求。
func (r *ProfileMFASecretRefreshReq) Validate() error {
	return nil
}

// ProfileAvatarReq 表示个人中心更新头像请求。
type ProfileAvatarReq struct {
	Avatar string `json:"avatar"` // 头像地址
}

// Validate 校验更新头像请求。
func (r *ProfileAvatarReq) Validate() error {
...
// ProfileTwoStepResp 表示前端两步校验票据。
type ProfileTwoStepResp struct {
...
// ProfileMFACheckResp 表示 MFA 校验接口响应。
type ProfileMFACheckResp struct {
	...
	TwoStep     *ProfileTwoStepResp `json:"twoStep"`     // 二次校验票据
}

// ProfileRoleInfo 表示前端使用的角色与权限响应。
...
// ProfileInfo 表示前端 `mine/login` 接口使用的用户信息。
...
// ProfileLoginResp 表示前端登录响应。
type ProfileLoginResp struct {
	Token string       `json:"token"` // 访问令牌
	User  *ProfileInfo `json:"user"`  // 当前登录管理员信息
}

// MFAURLResp 表示生成 MFA 绑定地址响应。
type MFAURLResp struct {
	BuildMFAURL string `json:"buildMFAURL"` // MFA 绑定地址
}
