...
type SysConfigExcelImportReq struct {
	UploadID string `json:"uploadId" form:"uploadId,optional"` // 已上传的 Excel 文件会话 ID
	BackupID string `json:"backupId" form:"backupId,optional"` // 当前上传文件对应的导入前备份 ID
}
...
// SysConfigExcelBackupReq 表示生成字典导入前备份的请求。
type SysConfigExcelBackupReq struct {
	UploadID string `json:"uploadId" form:"uploadId,optional"` // 待导入 Excel 的上传会话 ID
}

// Validate 校验字典导入前备份请求。
func (r *SysConfigExcelBackupReq) Validate() error {
	r.UploadID = strings.TrimSpace(r.UploadID)
	if r.UploadID == "" {
		return errors.Errorf("uploadId 不能为空")
	}
...
// SysConfigExcelBackupDownloadReq 表示字典导入前备份下载请求。
type SysConfigExcelBackupDownloadReq struct {
	BackupID     string `json:"backupId,optional" form:"backupId,optional"` // 参与签名的 query 备份 ID
	PathBackupID string `json:"-" form:"-" path:"backupId"`                 // 路径中的备份 ID
}

// Validate 校验字典导入前备份下载请求。
func (r *SysConfigExcelBackupDownloadReq) Validate() error {
	r.BackupID = strings.TrimSpace(r.BackupID)
	r.PathBackupID = strings.TrimSpace(r.PathBackupID)
	if r.BackupID == "" || r.PathBackupID == "" {
		return errors.Errorf("backupId 不能为空")
	}
	if r.BackupID != r.PathBackupID {
		return errors.Errorf("path 与 query 的 backupId 不一致")
	}
	return nil
}

// SysConfigExcelBackupResp 表示字典导入前备份结果。
type SysConfigExcelBackupResp struct {
	BackupID  string `json:"backupId"`  // 备份 ID
	FileName  string `json:"fileName"`  // 用户下载文件名
	ExpiresAt string `json:"expiresAt"` // 服务器备份过期时间
}
...
const (
	// SysConfigExcelBackupCleanupTaskType 表示字典导入前备份清理任务类型。
	SysConfigExcelBackupCleanupTaskType = "sys_config:excel:backup:cleanup"
)

// SysConfigExcelBackupCleanupTaskPayload 表示字典导入前备份清理任务负载。
type SysConfigExcelBackupCleanupTaskPayload struct {
	BackupID  string `json:"backupId"`  // 需要清理的备份 ID
	ObjectKey string `json:"objectKey"` // 需要删除的私有存储对象 key
}
