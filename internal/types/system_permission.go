...
// UpdatePermissionReq 表示编辑权限基础资料请求。
type UpdatePermissionReq struct {
	ID          int     `path:"id" json:"id,optional" form:"id,optional"` // 权限 ID，编辑时由路径注入
	UUID        string  `json:"uuid,optional"`                            // 权限 UUID；未传时保持原值
	Title       string  `json:"title"`                                    // 权限名称
	Module      *string `json:"module"`                                   // 权限匹配模块，允许显式传空字符串清空
	Pid         *int    `json:"pid"`                                      // 父级权限 ID，编辑时必须明确提交
	Type        *int    `json:"type"`                                     // 权限类型，编辑时必须明确提交
	Description *string `json:"description"`                              // 权限描述，允许显式传空字符串清空
}

// CreatePermissionReq 表示新增权限请求。
type CreatePermissionReq struct {
	UUID        string `json:"uuid,optional"`        // 权限 UUID，省略时由后端生成
	Title       string `json:"title"`                // 权限名称
	Module      string `json:"module,optional"`      // 权限匹配模块
	Pid         int    `json:"pid,optional"`         // 父级权限 ID
	Type        *int   `json:"type"`                 // 权限类型，新增时必须明确提交
...
func (r *CreatePermissionReq) Validate() error {
	...
	if r.Type == nil || *r.Type < 0 || *r.Type > 8 {
		return errors.Errorf("权限类型不合法")
	}
	if r.Status != nil && (*r.Status != 0 && *r.Status != 1) {
		return errors.Errorf("权限状态不合法")
...
// TypeValue 返回新增请求明确提交的权限类型。
func (r *CreatePermissionReq) TypeValue() int {
	if r == nil || r.Type == nil {
		return 0
	}
	return *r.Type
...
func (r *UpdatePermissionReq) Validate() error {
	...
	if r.Module == nil {
		return errors.Errorf("必须提交权限模块")
	}
	if r.Description == nil {
		return errors.Errorf("必须提交权限描述")
	}
...
// ParentID 返回编辑请求明确提交的父级权限 ID。
func (r *UpdatePermissionReq) ParentID() int {
	if r == nil || r.Pid == nil {
		return 0
	}
	return *r.Pid
...
// TypeValue 返回编辑请求明确提交的权限类型。
func (r *UpdatePermissionReq) TypeValue() int {
	if r == nil || r.Type == nil {
		return 0
	}
	return *r.Type
...
type AdminPermissionItem struct {
	...
	Manageable      bool                  `json:"manageable"`      // 当前管理员是否有权管理该权限
	CanCreateChild  bool                  `json:"canCreateChild"`  // 当前管理员是否可在该权限下新增子权限
