...
// TestDeleteSessionRemovesReuseIndexes 验证业务消费上传结果后不会复用已经失效的完成会话。
func TestDeleteSessionRemovesReuseIndexes(t *testing.T) {
	manager, _ := newTestUploadManager(t)
	ctx := context.Background()
	req := InitUploadReq{
		OperatorID:  3,
	...
	session, err := manager.Init(ctx, req)
	if err != nil {
		t.Fatalf("初始化上传会话失败: %v", err)
	}
	session.Status = "completed"
	session.ObjectKey = "excel-import/202607/26/" + session.StoredFileName
	if err := manager.PersistSession(ctx, session); err != nil {
		t.Fatalf("保存完成会话失败: %v", err)
	}
	if err := manager.DeleteSession(ctx, session); err != nil {
		t.Fatalf("删除已消费会话失败: %v", err)
	}
	if exists, err := manager.HasSession(ctx, session.UploadID); err != nil || exists {
		t.Fatalf("已消费会话仍然存在 exists=%t err=%v", exists, err)
	}
	recreated, err := manager.Init(ctx, req)
	if err != nil {
		t.Fatalf("重新初始化上传会话失败: %v", err)
	}
	if recreated.UploadID == session.UploadID {
		t.Fatalf("已消费文件不应复用旧 uploadId: %s", recreated.UploadID)
	}
	if _, err := manager.FindSessionByObjectKey(ctx, session.ObjectKey); !errors.Is(err, ErrUploadSessionNotFound) {
		t.Fatalf("已消费对象反查索引仍然存在: %v", err)
	}
}

// TestLoadSessionErrorClassification 验证会话缺失和 Redis 故障不会混为同一种业务错误。
func TestLoadSessionErrorClassification(t *testing.T) {
	manager, _ := newTestUploadManager(t)
	ctx := context.Background()
	if _, err := manager.GetSession(ctx, "missing"); !errors.Is(err, ErrUploadSessionNotFound) {
		t.Fatalf("缺失会话错误分类错误: %v", err)
	}
	if err := manager.client.Close(); err != nil {
		t.Fatalf("关闭测试 Redis 客户端失败: %v", err)
	}
	if _, err := manager.GetSession(ctx, "missing"); !errors.Is(err, ErrUploadSessionStoreUnavailable) {
		t.Fatalf("Redis 故障错误分类错误: %v", err)
	}
}
